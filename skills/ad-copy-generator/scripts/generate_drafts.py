...
def variants_for_vertical(dataset: dict[str, Any],
    ...
    CPL ascending (winners first). Includes ALL variants — caller
    ...
    # Sort each dimension by CPL ascending; null-CPL variants
    # sink to the bottom.
    for dim in by_dim:
        by_dim[dim].sort(
            key=lambda x: (x.get("cpl") if x.get("cpl") is not None
                            else float("inf"))
        )
    return by_dim
...
def vertical_summary(dataset: dict[str, Any], vertical: str) -> dict[str, Any]:
    ...
    ad_count = len(ads)
    cpls = [a["cpl"] for a in ads if a.get("cpl") is not None]
    spend_total = sum(a.get("spend") or 0 for a in ads)
    ic_total = sum(a.get("ic_conversions") or 0 for a in ads)
    try:
        median_cpl = round(median(cpls), 2) if cpls else None
    except StatisticsError:
        median_cpl = None
    return {
        "vertical": target,
        "ad_count": ad_count,
        "ads_with_conversions": len(cpls),
...
def format_variant_excerpts(variants: list[dict[str, Any]],
    ...
    for v in variants[:limit]:
        cpl = v.get("cpl")
        cpl_str = f"${cpl}" if cpl is not None else "no leads yet"
        out.append(
            f'  - "{v.get("text", "")}"  '
...
def build_prompt(vertical: str,
    ...
    user = (
        ...
        f"- median CPL: "
        f"{'$' + str(summary['median_cpl']) if summary['median_cpl'] is not None else 'n/a'}\n"
        f"- total spend: ${summary['spend_total']}\n"
        f"- total IC conversions: {summary['ic_total']}\n\n"
        "## Top-performing bodies (lowest CPL, real corpus quotes)\n"
        ...
        "## Bottom bodies (highest CPL — patterns to avoid)\n"
...
def render_markdown(vertical: str, summary: dict[str, Any],
    ...
    if summary.get("median_cpl") is not None:
        body.append(f"- median CPL: **${summary['median_cpl']}**")
...
def generate_for_vertical(vertical: str, dataset: dict[str, Any],
    ...
    # Split each dimension at its median CPL — variants below
    ...
    # ascending CPL includes the WORST variant. Median split
    # guarantees winners and losers are always distinct cohorts.
    winners: dict[str, list[dict[str, Any]]] = {}
    losers: dict[str, list[dict[str, Any]]] = {}
    for dim, variants in by_dim.items():
        valid = [v for v in variants if v.get("cpl") is not None]
        if len(valid) < 2:
            winners[dim] = valid[:5]
            losers[dim] = []
            continue
        try:
            med = median(v["cpl"] for v in valid)
        except StatisticsError:
            winners[dim] = valid[:5]
            losers[dim] = []
            continue
        winners[dim] = sorted(
            [v for v in valid if v["cpl"] < med],
            key=lambda v: v["cpl"])[:5]
        losers[dim] = sorted(
            [v for v in valid if v["cpl"] > med],
            key=lambda v: v["cpl"], reverse=True)[:5]

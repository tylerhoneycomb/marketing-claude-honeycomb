...
_Last updated: 2026-09-15 (**Strategic reallocation retired; the Tuesday brief is diagnosis-only and renders ACTIVE verticals only.** `compute_scaling_profiles.py` splits its output — `verticals` now holds only verticals with `active_campaign_count > 0`, a new `inactive_verticals` key carries the all-paused remainder (retained for history, never rendered), and the stdout summary counts active verticals and adds `inactive_vertical_count`. `agent-portfolio-scaling.yml` drops the `Compute reallocation` step (`compute_reallocation.py --write-log`), commits only `scaling_profiles.json`, stops passing `EXEC_SHARED_SECRET` to the `Compose brief` step, and its prompt is rewritten to a two-section diagnosis brief (scaling labels, audience actions): no pool, no `scaling-queue-write` registration, no approve/reject links, no last-week evaluation, no `portfolio.median_cpl` in the headline, status one-liner ending `diagnosis_only`. `Code.js` gains `STRATEGIC_SCALING_RETIRED = true` beside `BUDGET_OPTIMIZER_PAUSED`: `executeStrategicChanges()` early-returns and deletes `SCALING_PENDING_TOKEN` / `SCALING_APPROVED_TOKEN` / `SCALING_REJECTED_TOKEN` / `SCALING_LOCKOUT_UNTIL` / `SCALING_AFFECTED_CAMPAIGN_IDS`, and `doGet`'s `approve_scaling` / `reject_scaling` / `confirm_approve_scaling` / `confirm_reject_scaling` short-circuit to a "Strategic reallocation is retired" page above the existing `*_scaling` branches — a stale Slack approve link can no longer record an approval. `compute_reallocation.py` is kept on disk, unwired, with a RETIRED header docstring; `data/derived/reallocation.json` was deleted from the repo. Two side effects stopped with it: the `?action=scaling-write` POST, so `scaling_log` accretes no further rows (the weekly classification record is the git history of `data/derived/scaling_profiles.json`), and `audience_actions`, which the brief now derives from `new_audience_needed`. Stale Slack copy naming the Tuesday reallocation was removed from `buildBudgetWeeklySummary_`'s footer and the four spend-target strings. Prior: 2026-09-14 **Slack is leads-only.** Every Slack-bound string and every prompt that composes one drops the secondary IC line and any IC / ICP / CPICP / rewards / lead-subtype mention. `Code.js`: `postDailyDigest` and `postWeeklyNarrativeToSlack_` lose their `_of which reached an IC decision_` line and the `ydIC` / `wtdIC` / `d30IC` accumulators that fed it (`postWeeklyNarrativeToSlack_` drops its trailing `icConversions` param in lockstep with its single caller; `generateNarrativeForWeek_` still sums `totalICConversions` for the `intelligence_log` attribution rate but no longer passes it, puts it in the context block or renders `| IC decisions N` per campaign); `computeLeadPace_` no longer sums `icConversions` (the per-campaign signal in `computeBudgetSignals_` is untouched) and `runBudgetAnalysis`'s no-change notice and `postBudgetProposalToSlack_` drop the IC line and the "IC decisions in window" context line; the three Slack-composing system prompts replace their term-listing guardrails with "Report leads, CPL, spend and delivery diagnostics only. Never split leads into categories and never name any other conversion metric"; `syncCampaignMappings_`'s new-mapping alert drops the `(optimizes for: <custom conversion name>)` suffix (the name is still written to `campaign_mapping`); the Monday footer no longer says "(IC ranking retired)". Skills: `check_funnel_conversions` now returns a Slack-facing `slack_status` / `slack_detail` pair (quality tier only) beside the full `status` / `detail`, and `report_health.slack_lines` prefers the `slack_*` pair, so a subtype problem is WARN in the Sheet / issue but never a Slack line (§10.4); the daily-check, fatigue-monitor, creative-intelligence and portfolio-scaling SKILL.md files and workflow prompts state a leads-only Slack rule. Unchanged: Sheet columns, the issue-#48 one-liners (`ic=N`), the dashboard IC tab, the Hive Mind prompt, `intelligence_log` rows and every `/exec` wire contract (legacy IC-named keys still sent). Prior: 2026-09-10 **lead-first rewrite of every Slack-bound message and prompt.** `Code.js`: the daily digest and weekly narrative headline leads / CPL / spend with one secondary IC line, sort campaigns by CPL and key every alert on the new `TARGET_CPL_DOLLARS` / `CPL_*` constants (mirrored from `benchmarks.json`); `computeBudgetSignals_` / `computeRecommendations_` / `postBudgetProposalToSlack_` rank on CPL and lead trend (`computeWeeklyICPPace_` → `computeLeadPace_`, `PUMP_CPICP_CEILING` → `PUMP_CPL_CEILING`, HubSpot read dropped) while the `runBudgetAnalysis` / `executeBudgetChanges` guards stay and now clear the stale `BUDGET_*` tokens and `BUDGET_RANK_TIERS`; `buildBudgetWeeklySummary_(leads, cpl)` splits outcomes by `source` and reads `BUDGET_OPTIMIZER_PAUSED` for its footer; the `*_scaling` branches in `doGet` were moved above the `BUDGET_PENDING_TOKEN` gate that had made every Tuesday approve/reject link dead; strategic notices use status-aware lockout copy (`scalingLockoutStatusLine_`), execution summaries carry `signal_reasons`; the Hive Mind system prompt is lead-first. Skills: every SKILL.md + workflow prompt states the same standard (title says Leads, headline leads/CPL, CPL sorts, IC only as "of which N reached an IC decision"); fatigue-monitor adds leads/CPL to baselines and CPL inflation to the matrix; pipeline-health's `snapshot_volume` reports the lead total and Slack lines carry the check name; portfolio-scaling sums ACTIVE budgets only, buckets `LEADS-*` campaigns into their vertical, gates eligibility on lifetime leads and emits a 12-week portfolio headline; creative-intelligence and portfolio-scaling `claude-code-action` steps now receive `EXEC_SHARED_SECRET`. Wire contracts unchanged — legacy IC-named Sheet keys still sent; `scaling_log` still has no CPL column. Prior: 2026-09-09 **lead-first pivot of the ad-level pipeline, the six skills and the dashboard**, plus three follow-ups in `Code.js`. `ic_tracking` is replaced by a three-tier `conversions` spec read through `FunnelSpec`/`funnel_from_config()`; ranking, classification, reallocation weighting and creative deciles all move from CPICP to CPL. **`runBudgetAnalysis()` and `executeBudgetChanges()` are early-returned** — the optimizer ranked on CPICP, which fired once in the 30 days to 2026-09-08; `executeStrategicChanges()` is deliberately not guarded and stays live on the separate `SCALING_*` tokens. **A shared-secret gate (`PROTECTED_EXEC_ACTIONS` / `execAuthFailure_`) now covers every side-effecting `/exec` action** on both `doGet` and `doPost`, failing closed when `EXEC_SHARED_SECRET` is unset; read-only actions and the token-authenticated Slack approve/reject links are unaffected. Pacing target set to $2,100/week. Prior: 2026-06-23 folded pipeline-health into `daily-data.yml`; 2026-06-10 PAUSED `agent-fatigue-monitor.yml`)_
...
│   ├── portfolio-scaling/       # NEW (2026-05-08) compute_scaling_profiles.py
│   │                            # (compute_reallocation.py retired 2026-09-15, unwired)
...
| 3 | execution_scheduled | Date | Tomorrow at 3 AM (optimizer); next Wed 3 AM (strategic — historical only, no rows written since 2026-09-15) |
...
| 12 | source | Enum | `optimizer` (default; daily budget optimizer) or `strategic` (weekly portfolio-scaling reallocation). Legacy rows from before column 12 existed are read as `optimizer`. **No new `strategic` rows since 2026-09-15** — the producer is retired (§8.7); existing rows are unchanged and still counted by the 12%-cap headroom math. |

- **Writers:** `writeToQueue_(recommendations, source)` (`source` defaults to `'optimizer'`) for the daily optimizer; `handleScalingQueueWrite_(e)` for the Tuesday strategic proposal (no caller since 2026-09-15). Both are inert today — the optimizer is paused and the reallocation retired.
...
- **State machine driven by Script Properties:**
  ...
  - Strategic: `SCALING_PENDING_TOKEN`, `SCALING_APPROVED_TOKEN`, `SCALING_REJECTED_TOKEN`. On strategic execution, `SCALING_PENDING_LOCKOUT_UNTIL` and `SCALING_PENDING_AFFECTED_IDS` were promoted to `SCALING_LOCKOUT_UNTIL` and `SCALING_AFFECTED_CAMPAIGN_IDS` (the live keys read by the daily optimizer's lockout filter). _(Retired 2026-09-15: the guarded `executeStrategicChanges` deletes the three tokens and the two live lockout keys on every run instead of promoting anything.)_
- See §7 Budget Automation and §8 Portfolio Scaling.

### 3.7 `scaling_log` — Per-vertical scaling classifications

**16 columns. One row per vertical per Tuesday run. Append only.** _(added 2026-05-08; **no longer written as of 2026-09-15**)_

> **Frozen tab.** The only writer was `compute_reallocation.py --write-log`, which was unwired when strategic reallocation was retired (§8.7). `handleScalingWrite_` and `getScalingLogRows_` still exist and the existing rows are still readable — the Hive Mind chat's scaling-keyword path still reads them — but nothing appends. The weekly per-vertical classification record is now the git history of `data/derived/scaling_profiles.json`, committed on every Tuesday run.
...
- **Writer:** `handleScalingWrite_(e)` (POST `?action=scaling-write`). Auto-creates the tab on first call. **No caller since 2026-09-15** — the handler is live but unreached.
...
- **No CPL or lead column** _(known gap, 2026-09-10)_: the only cost columns are `ic_rate` / `cpicp` (cols 5-6). `compose_scaling_log_rows` sends `cpl` and `total_leads` in every row, but `handleScalingWrite_` reads keys by name and has no column for them, so they are dropped. Moot in practice since 2026-09-15: the brief no longer evaluates last week, so it makes no `?action=rollup` call and reads no cost series at all, and no new rows arrive to carry the missing columns. Extending the header only matters if the reallocation is revived. Tracked in §10.4.
...
| `BUDGET_OPTIMIZER_PAUSED` | `true` | Status flag only — the pause itself is the early returns in `runBudgetAnalysis` / `executeBudgetChanges`. Drives the Monday budget block footer, the strategic lockout / reject copy (`scalingLockoutStatusLine_`, `scalingRejectStatusLine_`, both unreachable since 2026-09-15) and the dashboard's `run_budget_analysis` response. Flip it when the guards come out. |
| `STRATEGIC_SCALING_RETIRED` | `true` | _(added 2026-09-15, Code.js:100, next to `BUDGET_OPTIMIZER_PAUSED`)_ Strategic (portfolio-scaling) reallocation is retired. Like the pause flag it drives copy and branching, not the stop itself: the hard stops are the early return in `executeStrategicChanges()` and the `*_scaling` short-circuit in `doGet` (§8.7, §9.1). Flip it — and restore the producer in `.github/workflows/agent-portfolio-scaling.yml` — to revive the feature. |
...
| `SCALING_MAX_WEEKLY_PCT` | `0.12` | Total \|change_pct\| per campaign per week, summed across optimizer + knockdown + strategic. Hard rail — still enforced unconditionally in `computeRecommendations_`, though since 2026-09-15 no strategic movement contributes to it (and the optimizer is paused, so today nothing does). **Intentionally dual-source with `data/config/benchmarks.json:scaling.max_weekly_total_change_pct`** — Python scripts read JSON, Apps Script reads this constant. Change both if you ever change one. |
...
| `SCALING_PENDING_TOKEN` | Active strategic-reallocation proposal token (one at a time, separate from optimizer's token). **Deleted on every 3 AM run since 2026-09-15** — see `executeStrategicChanges` (§8.7). |
| `SCALING_APPROVED_TOKEN`, `SCALING_REJECTED_TOKEN` | Strategic approve/reject state. Also deleted on every 3 AM run since 2026-09-15; nothing can set them while the `*_scaling` actions are short-circuited. |
...
| `SCALING_LOCKOUT_UNTIL` | ISO timestamp; daily optimizer skips affected campaigns until this passes. Deleted on every 3 AM run since 2026-09-15 (with both paths off, a surviving window would strand campaigns behind a gate nothing would lift). |
| `SCALING_AFFECTED_CAMPAIGN_IDS` | Comma-separated list of locked-out campaign IDs. Deleted alongside `SCALING_LOCKOUT_UNTIL` since 2026-09-15. |
...
**Four production call sites** (plus `testAnthropicConnection()`), all lead-first since 2026-09-10 — each prompt names leads and CPL as the metrics and states the `TARGET_CPL_DOLLARS` target. The three that compose Slack text are leads-only since 2026-09-14: their guardrail reads "Report leads, CPL, spend and delivery diagnostics (CTR, frequency, CPM, pacing) only. Never split leads into categories and never name any other conversion metric" (the earlier sentences that named IC / ICPs / CPICP inside the prompt are gone, and their context blocks carry no IC figure). `handleChatRequest_` serves the dashboard, not Slack, and keeps its IC-subtype guidance:
...
**Lead-first standard** _(applied 2026-09-10 to every message below and to every skill brief in §11.6)_: the headline of any Slack message is leads and CPL (and spend); titles say Leads; every ranking or ordering is by CPL (zero-lead rows last, or first where the point is "spend without leads"); every alert threshold is a CPL rule (`TARGET_CPL_DOLLARS × CPL_CRITICAL_MULTIPLE`, `CPL_SPIKE_WARNING_PCT`) gated on a minimum lead count (`CPL_FLAG_MIN_WEEKLY_LEADS` / `CPL_FLAG_MIN_DAILY_LEADS`); and no message may describe the daily optimizer as running while `BUDGET_OPTIMIZER_PAUSED` is true. **Leads-only** _(tightened 2026-09-14)_: a Slack message may mention leads, CPL, spend and delivery diagnostics (CTR, frequency, CPM, pacing) and nothing else — IC / ICP / CPICP, investment or rewards crowdfunding and any subtype of leads never appear, in any position (headline, secondary line, parenthetical, trailing token, "reported only" clause or title), and the prompt that composes a message must not name them either; the 2026-09-10 allowance of one secondary `_of which N reached an IC decision_` line is withdrawn. `prequal_decisions` may stay where it already appears. Sheet columns, the issue-#48 one-liners, the dashboard IC tab, the Hive Mind prompt, `intelligence_log` rows and the `/exec` wire contracts are not Slack and are unchanged. **Stale-state copy** _(extended 2026-09-15)_: no message may describe strategic reallocation as something that will happen while `STRATEGIC_SCALING_RETIRED` is true. The four strategic rows below can no longer fire at all, and the sentences in `buildBudgetWeeklySummary_`'s footer and the four spend-target strings that named the Tuesday reallocation were rewritten the same day.

**Where Slack messages are posted from (all via `postToSlack_` unless noted):**

| Message | Function | Trigger / status | Content (2026-09-10) |
|---|---|---|---|
| `*Honeycomb Leads — <EEE MMM d>*` daily digest | `postDailyDigest` (Code.js:~2187) | Daily 7 AM via `runDailyPipeline` — live | Yesterday / WTD / 30-day rows `Spend \| Leads \| CPL` (+ Freq on yesterday); `_Best CPL → … · Worst → …_` from `ydByCampaign`; WTD `WoW ±% (CPL)` vs prior rollup week and `Pacing → $X spend vs $T target \| N leads (d of 7 days)` with `T = getTargetWeeklySpend_()`; 30-day `(target $16)` and `Run rate → $X/week \| N leads/week`; watch line precedence: spend > `ZERO_LEAD_SPEND_WATCH_DOLLARS` with 0 leads → CPL > `PUMP_CPL_CEILING` on ≥ `CPL_FLAG_MIN_DAILY_LEADS` leads → freq > `FREQ_HIGH_THRESHOLD`; 2-sentence LLM commentary; source footer. The HubSpot `ydICPs` read was removed 2026-09-10; the `_of which reached an IC decision_` line and its `ydIC` / `wtdIC` / `d30IC` accumulators (rolling_data col 12 / rollup col 10) were removed 2026-09-14. |
| `*Honeycomb Leads — Week of <week>*` weekly post | `postWeeklyNarrativeToSlack_` (Code.js:~2089) | Mondays 8 AM via `generateWeeklyNarrative` — live | Yesterday / week / last-30-days rows `Spend \| Leads \| CPL`, week row shows `(target $16)` and `WoW ±% (prior $X CPL)`, 30-day row `vs prior 30` on CPL; the narrative; then the Budget Activity block. No IC line since 2026-09-14 (the `ydIC` / `d30IC` accumulators went with it). Signature: trailing `leads` arg added 2026-09-10; the `icConversions` arg that followed it was dropped 2026-09-14 in lockstep with its single caller; the older `icps, cpicp, attrICPs, attrRate` are still passed but not rendered. |
| Budget Activity block (embedded in the weekly post) | `buildBudgetWeeklySummary_(leads, cpl)` (Code.js:~4184) | Every Monday, non-null when `budget_queue` has rows | `Outcome this week: N leads at $X CPL (target $16)`; outcome counts split by `source` (`Daily optimizer: …` / `Strategic reallocation (portfolio-scaling): …`, with `expired` now counting rows marked `expired` as well as `failed` / `pending`); net changes per campaign tagged `_optimizer_` / `_strategic_`; anticipated spend from live Meta budgets; footer reads `BUDGET_OPTIMIZER_PAUSED` — `_Daily optimizer paused 2026-09-09; strategic reallocation retired 2026-09-15. No automated budget changes._` (the "(IC ranking retired)" parenthetical was dropped 2026-09-14; the "strategic reallocation via portfolio-scaling remains active (Tuesdays)" clause was replaced 2026-09-15) or `_Next CPL-ranked proposal: tomorrow morning_`. |
| `*Honeycomb Budget Proposal — <date>*` | `postBudgetProposalToSlack_(recs, token, leadPace, replacedPrior)` (Code.js:~3747) | 6 AM `runBudgetAnalysis` — **paused** (early return) | `*Leads — rolling 14 days*` `N leads \| CPL $X \| $S spend` (no IC line since 2026-09-14); Budget Pool; Reductions sorted worst-CPL-first (null CPL = 0 leads on top) and Increases best-CPL-first, each row `$cur → $prop/day (±%) \| CPL $X on N leads (14d)` (`budgetCplLabel_`) + `_reasons_`; AI commentary; approve/reject links. `allBudgets` param removed. |
...
| ~~`*Honeycomb Scaling* ✅ Strategic reallocation approved by …`~~ | `doGet` `confirm_approve_scaling` | **RETIRED 2026-09-15 — cannot fire.** The `STRATEGIC_SCALING_RETIRED` short-circuit above the `*_scaling` branches returns an HTML "Strategic reallocation is retired" page and posts nothing (§9.1). | Was `Executes at the next 3:00 AM run (N campaigns).` (`countPendingQueueRows_(token, 'strategic')`) + `scalingLockoutStatusLine_(SCALING_PENDING_LOCKOUT_UNTIL)`. |
| ~~`*Honeycomb Scaling* ❌ Strategic reallocation rejected by …`~~ | `doGet` `confirm_reject_scaling` | **RETIRED 2026-09-15 — cannot fire.** Same short-circuit. | Was `No reallocation this week.` (HTML page added `scalingRejectStatusLine_()`). |
| ~~`*Honeycomb Scaling — <date>*` rejected / expired~~ | `executeStrategicChanges` | **RETIRED 2026-09-15 — cannot fire.** The 3 AM trigger still runs but the function early-returns above `validateTokens_()`, logs, and deletes the five `SCALING_*` properties. | Was `⏰ No approval received by the 3:00 AM run. Reallocation expired.` + `_N campaigns, net ±$X/day_`. |
| ~~`*Honeycomb Scaling — Strategic Reallocation Applied (<date>)*`~~ | `postStrategicExecutionSummaryToSlack_` | **RETIRED 2026-09-15 — cannot fire.** Only caller is `executeStrategicChanges` below its guard. | Was per campaign `↑/↓ name: $cur → $prop/day — <vertical>: <classification> \| CPL $X on N leads \| <reason>` (the queue row's `signal_reasons`), failures, then `scalingLockoutStatusLine_(lockoutUntil)`. |
| `*Honeycomb Spend Target Change*` proposal + ✅ / ❌ confirmations | `handleDashboardApi_` `propose_spend_target` (Code.js:~5150), `applyTargetDecision_` (Code.js:~6812) | Dashboard action (gated) + human click | Dollar headline unchanged. _(2026-09-15)_ All four strings attribute the target to pacing alone: proposal `This is the weekly spend target used by pacing (and by the daily optimizer when it is re-enabled).`, approval page `Pacing uses the new target from its next run.`, approved notice `Takes effect on the next pacing check.`, approved page `Pacing uses this target from its next run.` The "and the Tuesday strategic reallocation" clause was removed with the reallocation. |
| Operational alerts: API failure (`fetchWithRetry_`), permission issue, new campaign mappings (campaign → utm per row; the `(optimizes for: <custom conversion name>)` suffix was dropped 2026-09-14 because in this account that name is a lead subtype — the name is still written to `campaign_mapping` col 3), renames, unresolvable campaigns (`excluded from the weekly rollup, the daily/weekly lead digests and any budget tooling until mapped`), names reconciled | Code.js:~234, `syncCampaignMappings_` | Daily sync — live | Metric-neutral; two stale phrasings fixed 2026-09-10; conversion-name suffix dropped 2026-09-14. |
...
| Daily, 3 AM | `executeStrategicChanges()` _(added 2026-05-08)_ | **RETIRED 2026-09-15.** Trigger still installed; the function logs, deletes `SCALING_PENDING_TOKEN` / `SCALING_APPROVED_TOKEN` / `SCALING_REJECTED_TOKEN` / `SCALING_LOCKOUT_UNTIL` / `SCALING_AFFECTED_CAMPAIGN_IDS`, and returns — above `validateTokens_()`, so it costs five property deletes and no API call. When live it applied approved strategic-reallocation rows, promoted the pending lockout keys and posted the execution summary. Re-enable = remove the guard and flip `STRATEGIC_SCALING_RETIRED` (and restore the producer — see §8.7). |
...
4. `postDailyDigest()` — the `*Honeycomb Leads — <date>*` Slack message: yesterday + WTD + last 30 days as `Spend | Leads | CPL` rows, pacing vs the live weekly spend target, best/worst CPL yesterday, a CPL-keyed watch line and a two-sentence LLM verdict (full shape in §6.2; the secondary IC line was removed 2026-09-14). Reads `rolling_data` and `weekly_rollup` only — the HubSpot `hubspot_icps` read was dropped on 2026-09-10.
...
- Builds `contextBlock` _(lead-first, 2026-09-10)_: `TOTAL LEADS`, `OVERALL CPL: $X (target $16)`, `TOTAL SPEND`; `CAMPAIGN BREAKDOWN (sorted by CPL, best first…)` with `spend | leads | CPL | WoW | vs 4wk avg | freq` per line (the `IC PREQUAL DECISIONS (… reported only)` line and the per-campaign `| IC decisions N` suffix were removed 2026-09-14 — the block carries no IC figure); then `CPL ABOVE TARGET (>$32 … campaigns with >=10 leads)`, `CPL SPIKE ALERTS (>25% above 4-week average; >=10 leads)`, `FREQUENCY ALERTS (>3)`, `SPEND WITHOUT LEADS`. All thresholds come from the `TARGET_CPL_DOLLARS` / `CPL_*` / `FREQ_HIGH_THRESHOLD` constants (§4.1), not literals.
- System prompt: community-investment-platform persona; "Primary metrics: leads, CPL, spend, frequency"; "Report leads, CPL, spend and delivery diagnostics (CTR, frequency, CPM, pacing) only. Never split leads into categories and never name any other conversion metric" _(2026-09-14 — replaces the two sentences that named IC prequal decisions, ICPs and CPICP inside the prompt)_; SEGMENTS format `- [utm]: [N] leads, $[CPL] CPL` + `- No leads: …`.
...
- If `postToSlack: true`: calls `postWeeklyNarrativeToSlack_(…, totalLeads)` — `totalICConversions` is still summed for the `intelligence_log` attribution rate but no longer passed (2026-09-14) — the `*Honeycomb Leads — Week of …*` post described in §6.2, which in turn embeds `buildBudgetWeeklySummary_(leads, cpl)`.
...
- `icConversions` — sum of col 12; carried on the per-campaign signal only — never enters a sort or threshold, and since 2026-09-14 neither summed into the lead pace nor rendered anywhere in Slack
- `avgFreq` — weighted average frequency

Returns `{campaignId: {spend, leads, cpl, leadTrend, icConversions, avgFreq, lifetimeConversions, …}}`.

**`computeLeadPace_(signals)`** (Code.js:~3034) sums those signals into the portfolio `{leads, spend, cpl, label}` (the `icConversions` sum was dropped 2026-09-14 with the Slack line it fed) used by the proposal headline and the no-change notice (label: `N leads | CPL $X | $S spend`). It replaced `computeWeeklyICPPace_`, which counted `hubspot_icps` rows.
...
| ~~`portfolio-scaling/compute_reallocation.py`~~ | read `scaling_profiles.json` (populated by the above). **Retired 2026-09-15 — unwired; no longer a consumer.** |
...
**Live override as of 2026-09-14: `DASHBOARD_TARGET_WEEKLY_SPEND = 9000`, `DASHBOARD_WEEKLY_SPEND_TOLERANCE = 50`** (`get_spend_goal` on the deployed script returned `target_weekly_spend: 9000, weekly_spend_tolerance: 50, source: script_property_override`). Because the override wins, the $2,100 / $315 target in `benchmarks.json` and the constants is **not in effect** until the two Script Properties are set to `2100` / `315` (Apps Script → Project Settings → Script Properties) or deleted so the constants apply, or a new target is approved through the dashboard's `propose_spend_target` flow. Until then `daily-check` pacing and the (paused) optimizer measure against $9,000 ± $50. `compute_scaling_profiles.py` still calls `get_spend_goal` and writes `portfolio.target_weekly_spend` / `weekly_spend_tolerance` / `tolerance_headroom_daily_cents` into `scaling_profiles.json`, but since 2026-09-15 nothing reads them: the pool they bounded is retired and the brief does not render a target. No code path resets the override on deploy — it is a deliberate dashboard-managed setting.

### 8.7 Portfolio scaling integration _(added 2026-05-08; reallocation retired 2026-09-15)_

> **Reallocation retired 2026-09-15.** The skill is diagnosis-only: it emits `scaling_profiles.json` and a Slack brief, and proposes nothing. `compute_reallocation.py` is unwired (kept on disk with a RETIRED header docstring), `data/derived/reallocation.json` was deleted, `handleScalingQueueWrite_` has no caller, and `executeStrategicChanges()` early-returns under `STRATEGIC_SCALING_RETIRED`. The three optimizer-side behaviors below are unchanged **as code**; the "Strategic execution path" section is kept as the revival reference and does not run today.
...
**1. Lockout filter (eligibility loop).** Campaigns whose IDs appear in `SCALING_AFFECTED_CAMPAIGN_IDS` while `SCALING_LOCKOUT_UNTIL` is in the future are excluded from `eligible[]` with a logged reason. The optimizer doesn't propose any change for them. _(Inert since 2026-09-15: the guarded `executeStrategicChanges()` deletes both properties on every 3 AM run, so `getScalingLockoutSet_()` always returns `{}`. Deleting them is deliberate — with both the proposer and the optimizer off, a surviving lockout window would strand campaigns behind a gate nothing would ever lift.)_
...
**3. Classification tagging (informational, gated on profiles).** When `scalingProfiles` is non-null, each eligible campaign gets `c.scalingClassification`, `c.scalingConfidence`, `c.scalingNewAudienceNeeded`, `c.scalingVertical` populated, and a tag string appended to `c.reasons` (e.g. `"breweries: scalable + new-audience-needed"`). No logic change — the tag surfaces in the Slack proposal text only. _(2026-09-15: `getVerticalClassification_` reads `profiles.verticals`, which now excludes verticals with no ACTIVE campaign — those moved to `inactive_verticals`, which no Apps Script reader consumes. Harmless for tagging, since only ACTIVE campaigns reach `eligible[]` and their vertical is by definition still in `verticals`; a revived reader that wants the full set must merge both keys. `profiles.campaigns` is unchanged and still carries every campaign, so `getCampaignVertical_` is unaffected.)_

**Strategic execution path — RETIRED 2026-09-15.** What runs today: `agent-portfolio-scaling.yml` runs `compute_scaling_profiles.py`, commits `data/derived/scaling_profiles.json` to main, and `claude-code-action` composes and posts a two-section diagnosis brief. Nothing is registered, nothing is approved, nothing executes. The flow below is the revival reference:

- **Tuesday AM** — `agent-portfolio-scaling.yml` ran `compute_scaling_profiles.py` + `compute_reallocation.py`, committed derived JSON to main, then `claude-code-action` POSTed the proposal to `/exec?action=scaling-queue-write`.
...
- **Tyler approves** — `doGet` action `approve_scaling` → `showScalingConfirmationPage_()` → `confirm_approve_scaling` → sets `SCALING_APPROVED_TOKEN`, **(all four actions short-circuited since 2026-09-15 — see §9.1)**, posts `*Honeycomb Scaling* ✅ … Executes at the next 3:00 AM run (N campaigns).` (`countPendingQueueRows_(token, 'strategic')`) plus `scalingLockoutStatusLine_(SCALING_PENDING_LOCKOUT_UNTIL)`. **Until 2026-09-10 this path was unreachable**: `doGet` checked `BUDGET_PENDING_TOKEN` (and `token === pendingToken`) before it reached the `*_scaling` branches, and scaling tokens are minted independently by `handleScalingQueueWrite_`, so every link from the Tuesday brief returned "No pending budget proposal" / "Token mismatch" and every strategic proposal expired at the next 3 AM run. The scaling branches now sit above that gate (§9.1).
- **Next 3 AM run** (the trigger is daily, not Wednesday-only) — `executeStrategicChanges()` ran (it now early-returns above all of this):
...
- **Lockout expires** at `SCALING_LOCKOUT_UNTIL` (typically next Tuesday 00:00 UTC), unblocking the optimizer's Tuesday morning cycle. _(Today the key is deleted on every 3 AM run instead.)_
...
- **`doGet(e)` (Code.js:~4475)** — handles all GET requests, in this order:
  ...
  3. **Strategic-reallocation retirement short-circuit** _(added 2026-09-15)_ — when `STRATEGIC_SCALING_RETIRED` is true, any of `approve_scaling` / `reject_scaling` / `confirm_approve_scaling` / `confirm_reject_scaling` returns an HTML "Strategic reallocation is retired." page and nothing else: no token read, no property write, no Slack post. Every such link is stale by definition — no brief mints a scaling token any more — and recording an approval would produce a Slack "executes at 3 AM" confirmation for a move `executeStrategicChanges` will discard.
  4. **Strategic-reallocation links** (unreachable while retired) — `approve_scaling` / `reject_scaling` → `showScalingConfirmationPage_`, `confirm_approve_scaling` / `confirm_reject_scaling` → token check against `SCALING_PENDING_TOKEN`, property writes, Slack post. These sit **above** the optimizer gate as of 2026-09-10; before that they were unreachable because the optimizer gate ran first and the scaling token never equals `BUDGET_PENDING_TOKEN`.
  5. Optimizer links — read `BUDGET_PENDING_TOKEN`; "No pending budget proposal" / "Token mismatch" pages; `approve` / `reject` → `showBudgetConfirmationPage_`; `confirm_approve` / `confirm_reject`.
  6. Spend-target links — `approve_target` / `reject_target` / `confirm_*_target`.
...
| `scaling-write` | POST or GET | JSON body `{rows:[...]}` or `rows=<json>` | Appends per-vertical rows to `scaling_log` tab (auto-created, see §3.7). Stores `ic_rate` / `cpicp`; drops the `cpl` / `total_leads` keys the script also sent. **No caller since 2026-09-15** — its only one was `compute_reallocation.py --write-log`, retired with the reallocation. |
| `scaling-queue-read` | GET | `since` (YYYY-MM-DD) optional, `campaign_id` optional, `source` optional (`optimizer` / `strategic`) | Returns `{rows: [...], count: N, since}` from `budget_queue`. Unlike `budget-queue-read` (which filters to `pending` only), this returns **all statuses** so callers can sum executed rows for headroom math. Each row: same fields as `budget-queue-read` plus `source`. Used by `compute_scaling_profiles.py` for the 12%-cap headroom math (still live). |
| `scaling-queue-write` | POST | JSON body `{rows: [{campaign_id, campaign_name, current_daily_cents, proposed_daily_cents, change_cents, change_pct, signal_reasons}, ...], lockout_until: "ISO", affected_campaign_ids: [...]}` | Writes pending strategic rows to `budget_queue` with `source='strategic'` and a fresh token. Stashes lockout metadata under `SCALING_PENDING_*` properties. Returns `{ok, token, written, approve_url, reject_url, lockout_until}`. **No caller since 2026-09-15** — the Tuesday workflow stopped registering proposals, and rows written here could no longer be approved (`doGet` short-circuit, §9.1) or executed (`executeStrategicChanges` guard, §8.7). |
| `scaling-log-read` | GET | `since` (YYYY-MM-DD) optional, `vertical` (slug) optional, `limit` (default 200, max 1000) optional | Returns `{rows: [...], count: N}` from `scaling_log`, newest-first. Used by Hive Mind chat for scaling-keyword queries. Still live, but the tab stops gaining rows on 2026-09-15 (§3.7), so the newest row it can return is from the last pre-retirement Tuesday. |
...
- Prompt _(lead-first 2026-09-10; it had still said "one line per campaign with IC conversions")_: run `fetch_daily_data.py > /tmp/daily_data.json` → `analyze_daily.py --input /tmp/daily_data.json` → compose the `📊 Daily Lead Check — <date>` summary: TOTALS line (`N leads · $X CPL · $Y spend · P prequal decisions` — the IC parenthetical was removed 2026-09-14), PACING, PORTFOLIO in JSON order (best CPL first, `0 leads` explicit; the `· IC n` suffix was removed 2026-09-14), WINNERS ranked by CPL (`$CPL, N leads`), BLEEDERS rendered by `reason`, FATIGUE WATCH, LEARNING, STALE; the prompt states the brief as a term-free allowlist — render nothing beyond the fields the format names; every other JSON field exists for the `daily_check_log` row (2026-09-14). Status one-liner: `pacing=… spend=$… leads=… cpl=$… winners=… bleeders=… fatigue_flags=…` (optional trailing `ic=`).
...
**`agent-portfolio-scaling.yml`** _(added 2026-05-08; **diagnosis-only since 2026-09-15**)_ — the only agent workflow still on a schedule.
...
- Pipeline _(2026-09-15)_: `compute_scaling_profiles.py` (receives `META_ACCESS_TOKEN` + `EXEC_SHARED_SECRET` for `get_spend_goal`) → commit `data/derived/scaling_profiles.json` to main BEFORE the action → `claude-code-action` composes and posts the brief → dump log → status comment. Removed with the retirement: the `Compute reallocation` step (`compute_reallocation.py --write-log`, which sized the pool, registered the proposal via `scaling-queue-write` and wrote the `scaling_log` rows), `reallocation.json` from the commit step's `git add`, and `EXEC_SHARED_SECRET` from the `Compose brief` step — that step now makes no side-effecting `/exec` call, so the `{"error":"unauthorized"}` fallback path and the `signal_reasons` / `portfolio knockdown` contract no longer apply to it.
- Prompt _(rewritten diagnosis-only 2026-09-15; lead-first 2026-09-10)_: title `*Honeycomb Scaling — <date>*`; open with `Portfolio (12w): N leads · $S spend · CPL $X` from `portfolio.total_leads / total_spend / cpl` — **`median_cpl` is no longer printed**, it spans every vertical the window ever saw and so compares live performance against retired campaigns — followed by `Active portfolio: $D/day ($W/wk) across N ACTIVE campaign(s).` from `portfolio.current_total_daily_cents` / `.active_campaign_count` (never add `paused_total_daily_cents`). **Two sections.** (1) Scaling labels: render every key in `verticals` and nothing else, CPL-ascending within class, `cpl / total_conversions / total_spend` per vertical, elasticity r, spend share, frequency and CPM trend, `directional` tagged; a vertical classified `insufficient` prints as a `too new to classify — CPL $X on N leads (k weeks, j active campaign(s))` one-liner after the classified list (live campaigns with too little history, not a problem to flag); `CPL —` for null CPL, last in its group, never another cost figure; `No ACTIVE campaigns to report this week.` when `verticals` is empty. (2) Audience actions: one line per `new_audience_needed` vertical — `CPL $X on N leads (12w)` diagnosis, then "duplicate ad set with broader targeting", then the frequency / CPM evidence — section skipped entirely when none qualify. The prompt explicitly forbids reading, rendering or mentioning `inactive_verticals`; naming, counting or alluding to a paused campaign or retired vertical; reading or mentioning `data/derived/reallocation.json` even if a stale copy is on disk; POSTing to any `/exec` action; and writing an approve or reject URL or otherwise proposing, implying or inviting a budget change. _(2026-09-14)_ Leads, CPL, spend and delivery diagnostics (frequency, CPM, elasticity) only — every other JSON field (`total_ic_conversions`, `ic_rate`, `cpicp`, `median_cpicp`, `median_ic_rate`) is never rendered in any position nor used for ordering. Status one-liner: `leads=<portfolio.total_leads> cpl=$<portfolio.cpl> verticals=<N active rendered> scalable=… saturating=… over_invested=… insufficient=… new_audience_needed=… diagnosis_only` — the pool tokens (`freed` / `allocated` / `net` / `knockdown_risk`) went with the reallocation.
- Data fixes shipped with the pass (`compute_scaling_profiles.py`): `VERTICAL_RE` accepts the `LEADS-` prefix so `LEADS-Broad-Q3-2026` → `broad` and `LEADS-IFW-Broad-Q3-2026` → `ifw-broad` (before, the only two delivering campaigns fell through to one-campaign "verticals" with < 6 weeks of history and were skipped as `insufficient`); `lifetime_leads_per_campaign` (sum of `meta_conversions`) replaces `lifetime_ic_per_campaign` for `optimizer_eligible`, mirroring `LIFETIME_MIN_CONVERSIONS`; `portfolio.current_total_daily_cents`, the tolerance headroom and `optimizer_cycles_this_week` count ACTIVE campaigns only (the paused-inclusive total read ~$24k/week against a $2,100 target and scaled every increase to zero with `knockdown_risk` every week); verticals are emitted CPL-ascending; the portfolio block gains `total_leads / total_spend / cpl / total_ic_conversions / active_campaign_count / paused_total_daily_cents`; per-vertical `active_campaign_count`. `compute_reallocation.py`: proposal rows carry `cpl / total_leads / vertical_spend` (the mis-indented `cpicp` key was removed), `pool_skip_reasons` explains an empty side of the pool, audience diagnoses open with `CPL $X on N leads (12w)`, and `scaling_log` rows also send `total_leads` (dropped by the handler). _(That script was retired on 2026-09-15; these fixes survive in the unwired file.)_
- Script change shipped with the retirement (`compute_scaling_profiles.py`): the output's `verticals` key now holds **only** verticals with `active_campaign_count > 0`; the all-paused remainder moves to a new top-level `inactive_verticals` key (identical per-vertical schema, retained for history and later analysis, never rendered). The stdout summary's `verticals` / `scalable` / `saturating` / `over_invested` / `new_audience_needed` all count active verticals only, and a new `inactive_vertical_count` reports the rest. Motivation: the 2026-09-15 brief led with three all-paused verticals tagged `over-invested` (warning icons and all) while the only two campaigns Meta was delivering appeared last. A vertical returns to `verticals` by itself as soon as one of its campaigns goes ACTIVE again. Apps Script readers see the narrowed set — see §8.7 behavior 3.
...
| `postWeeklyNarrativeToSlack_` | Code.js:~2089 | `*Honeycomb Leads — Week of …*` post. Trailing `leads` arg (`icConversions` dropped 2026-09-14); WoW and vs-prior-30 on CPL; no IC line; embeds `buildBudgetWeeklySummary_(leads, cpl)`. |
| `postDailyDigest` | Code.js:~2187 | `*Honeycomb Leads — <date>*` digest: `Spend \| Leads \| CPL` rows, pacing vs target, best/worst CPL, CPL-keyed watch line (IC line removed 2026-09-14), lead-first LLM commentary. |
...
| `computeBudgetSignals_` | Code.js:~2947 | 14-day rolling signals per campaign: `spend, leads, cpl, leadTrend, icConversions (carried, never rendered), avgFreq, lifetimeConversions`. `rolling_data` only — no HubSpot read. |
| `computeLeadPace_` | Code.js:~3034 | Portfolio `{leads, spend, cpl, label}` summed from the signals (`icConversions` sum dropped 2026-09-14); replaced `computeWeeklyICPPace_`. |
...
| `getScalingLockoutSet_` | Code.js:~3114 | `{campaign_id: true}` map of currently-locked-out campaigns (empty when lockout absent or expired). Always `{}` since 2026-09-15 — the guarded `executeStrategicChanges` deletes both lockout properties daily. |
...
| `getVerticalClassification_` | Code.js:~3186 | Reads a vertical's `classification / confidence / new_audience_needed` (+ `elasticity_r`, `spend_share_pct`, legacy `cpicp`) from the profiles for the optimizer tag. Reads `profiles.verticals` only — since 2026-09-15 that excludes verticals with no ACTIVE campaign (`inactive_verticals`), which is harmless for tagging but matters to any new reader (§8.7). |
...
| `postBudgetProposalToSlack_` | Code.js:~3747 | `(recs, token, leadPace, replacedPrior)`. Lead-pace headline (optional IC line removed 2026-09-14); reductions worst-CPL-first, increases best-CPL-first; AI commentary. Sub-dollar "reductions" render with `→ (held flat)` instead of `↓ (-0.1%)`. |
...
| `buildBudgetWeeklySummary_` | Code.js:~4184 | `(leads, cpl)`. Monday Budget Activity block: outcome line, per-source counts (`expired` includes rows marked `expired`), net changes tagged by source, anticipated spend, `BUDGET_OPTIMIZER_PAUSED` footer — `_Daily optimizer paused 2026-09-09; strategic reallocation retired 2026-09-15. No automated budget changes._` since 2026-09-15. |
| `checkExecSecret_` / `execAuthFailure_` | Code.js:~4444 / ~4462 | Shared-secret gate for `PROTECTED_EXEC_ACTIONS` (fails closed) |
| `doGet` | Code.js:~4475 | Web App GET router. Order: auth gate → dashboard actions → **`STRATEGIC_SCALING_RETIRED` short-circuit for `approve_scaling` / `reject_scaling` / `confirm_approve_scaling` / `confirm_reject_scaling` (2026-09-15, returns the "Strategic reallocation is retired." page)** → `*_scaling` links → `BUDGET_PENDING_TOKEN` gate + optimizer links → target links (scaling moved above the gate 2026-09-10). |
| `showBudgetConfirmationPage_` | Code.js:~4598 | HTML form with optional approver text input + confirm button. Form submits via GET. |
| `scalingLockoutStatusLine_` / `scalingRejectStatusLine_` | Code.js:~4662 / ~4673 | Status-aware lockout / reject sentences driven by `BUDGET_OPTIMIZER_PAUSED`. **Unreachable since 2026-09-15** — every caller is behind the retirement short-circuit or the `executeStrategicChanges` guard (§10.4). |
| `countPendingQueueRows_` | Code.js:~4681 | Pending `budget_queue` rows for a token (optional `source` filter) — was the "N campaigns" in the scaling approval message; unreached since 2026-09-15 |
| `showScalingConfirmationPage_` | Code.js:~4696 | Two-step approval HTML page for strategic reallocations. **Unreachable since 2026-09-15** — `doGet` answers `approve_scaling` / `reject_scaling` with the retirement page before this is called. |
| `executeStrategicChanges` | Code.js:~4804 | Strategic-reallocation execution entry point (daily 3 AM trigger). **RETIRED 2026-09-15** — early-returns under `STRATEGIC_SCALING_RETIRED` above `validateTokens_()`, logging and deleting `SCALING_PENDING_TOKEN` / `SCALING_APPROVED_TOKEN` / `SCALING_REJECTED_TOKEN` / `SCALING_LOCKOUT_UNTIL` / `SCALING_AFFECTED_CAMPAIGN_IDS`. The tokens are deleted so a stale approve/reject click cannot confirm a move that will never run; the lockout keys are deleted so no campaign is stranded behind a window nothing will lift. Below the guard the original orphan-expiry / `applyBudgetQueueRows_(source='strategic')` / lockout-promotion / Slack-summary body is intact for revival. |
| `postStrategicExecutionSummaryToSlack_` | Code.js:~4907 | `*Honeycomb Scaling — Strategic Reallocation Applied*` with per-row `signal_reasons` + status-aware lockout line. **Unreachable since 2026-09-15** — only called below the `executeStrategicChanges` guard. |
...
| `handleScalingQueueWrite_` | Code.js:~5657 | POST /exec?action=scaling-queue-write — writes strategic proposal rows + token + lockout metadata. Still routed and still gated by `PROTECTED_EXEC_ACTIONS`, but **no caller since 2026-09-15**; rows it wrote could no longer be approved (`doGet` short-circuit) or executed (`executeStrategicChanges` guard). |
| `handleScalingWrite_` | Code.js:~5749 | POST /exec?action=scaling-write — appends per-vertical scaling_log rows (`ic_rate` / `cpicp`; no CPL column). **No caller since 2026-09-15** — its only one was `compute_reallocation.py --write-log` (§3.7). |
...
| `showTargetApprovalPage_` / `applyTargetDecision_` | Code.js:~6744 / ~6812 | Spend-target approval page + decision. Copy references pacing only since 2026-09-15 (`Pacing uses the new target from its next run.` / `Takes effect on the next pacing check.` / `Pacing uses this target from its next run.`) |
...
| `scaling_log` stores only `ic_rate` / `cpicp` — no CPL, no lead count | `Code.js: handleScalingWrite_` ~5749, `getScalingLogRows_` ~5890; `compute_reallocation.compose_scaling_log_rows` | Added 2026-09-10. Consequence of the row above: the only week-over-week cost series the Sheet keeps for verticals is CPICP. The Tuesday brief therefore computes last week's CPL and lead movement from `?action=rollup` (`spend` / `meta_conversions` per vertical, two most recent `week_start`s) and uses `scaling-log-read` only for classification, frequency trend and pool flags. Fix = add `cpl` + `total_leads` columns to the header, the write and the read, then a redeploy. **Downgraded 2026-09-15:** the only writer (`compute_reallocation.py --write-log`) is retired, so the tab gains no rows and the brief no longer evaluates last week or calls `?action=rollup`. The gap is now inert history — only worth fixing if the reallocation is revived. |
| Apps Script pipeline still computes IC attribution under the hood | `Code.js: buildWeeklyRollup`, `IC_CONVERSION_EVENT_PATTERN`, `intelligence_log` cols 3-4 | Added 2026-09-09, narrowed 2026-09-10 and 2026-09-14. Every Slack message, prompt and ranking in `Code.js` is lead-first and, since 2026-09-14, leads-only, so nothing acts on, headlines or even mentions the IC numbers in Slack. But `buildWeeklyRollup` still runs the hybrid v3 attribution to fill rollup cols 10-13 / 15-21, `generateNarrativeForWeek_` still computes `totalICPs` / `overallCPICP` to keep `intelligence_log` cols 3-4 positionally stable for `getLatestNarrative_` / the dashboard, and the IC conversion event is still matched by fragile string pattern. Retiring those columns needs a header migration on both sheets plus dashboard changes. |
...
| ~~`creative-intelligence-read` action does not exist~~ | ~~`compute_reallocation.fetch_creative_intelligence_log`; `Code.js: handleDashboardApi_`~~ | Added 2026-09-10, **moot 2026-09-15.** The Tuesday brief's audience-action `creative_prescription` ("Top body: …") was sourced from a GET `?action=creative-intelligence-read` that no handler serves, so `fetch_json` always failed and the line was always null. Its only caller lived in the now-retired `compute_reallocation.py`, and the brief derives audience actions from `new_audience_needed` in `scaling_profiles.json` instead. The endpoint still does not exist; nothing asks for it. |
...
| `check_funnel_conversions` has a Slack verdict distinct from its Sheet / issue verdict | `skills/pipeline-health/scripts/check_health.py: check_funnel_conversions`; `report_health.py: slack_status` / `slack_detail` / `slack_lines` | Added 2026-09-14. Slack is leads-only, so the funnel check returns two views: `status` / `detail` (quality tier first, then `subtypes (reported only): …`) feed the `pipeline_health` Sheet row, the terminal summary and the issue-#48 one-liner, while `slack_status` / `slack_detail` cover the quality tier only and are what `report_health.slack_lines` posts (it prefers a check's `slack_*` pair when present, else `status` / `detail`). A subtype conversion that is archived or stops firing is therefore WARN in the Sheet and the issue but produces no Slack line at all — the only alerting path for a subtype tracking problem is now the Sheet tab and issue #48. It is the one check whose Slack verdict can differ from its Sheet verdict; any other check that later adds a `slack_*` pair inherits the same split, and a reader of `pipeline_health` should not expect a matching Slack alert for every WARN. |
| Retired reallocation code kept on disk, unwired | `skills/portfolio-scaling/scripts/compute_reallocation.py`; `Code.js: executeStrategicChanges` ~4804 + the `*_scaling` branches in `doGet`; `handleScalingQueueWrite_` ~5657, `handleScalingWrite_` ~5749 | Added 2026-09-15. Strategic reallocation was retired at Tyler's request but **not deleted**: `compute_reallocation.py` keeps the pool maths, elasticity-weighted sizing and lockout logic behind a RETIRED header docstring; `executeStrategicChanges` keeps its full body below the `STRATEGIC_SCALING_RETIRED` early return; the approval branches, `showScalingConfirmationPage_`, `postStrategicExecutionSummaryToSlack_`, `countPendingQueueRows_`, `handleScalingQueueWrite_` and `handleScalingWrite_` are all live code with no reachable caller. Deliberate — reviving the feature should be a workflow step plus a flag, not a rewrite — but it is dead code that will silently rot against the rest of the file (it already needs one fix: `compute_reallocation.py` reads the `verticals` key, which since 2026-09-15 excludes verticals with no ACTIVE campaign). Re-read every piece before trusting it. Revisit if the reallocation is still retired at the next major `Code.js` refactor. |
| `scalingLockoutStatusLine_` / `scalingRejectStatusLine_` are unreachable | `Code.js` ~4662 / ~4673 | Added 2026-09-15. Both helpers exist only to keep strategic copy honest about `BUDGET_OPTIMIZER_PAUSED`, and every caller now sits behind the retirement short-circuit in `doGet` or the `executeStrategicChanges` guard — so they are dead code on the revival path. Keep them with the rest of the strategic flow; if the reallocation is revived they must be re-checked against whatever the optimizer's state is then. |
...
5. For budget automation: while the optimizer is paused, `runBudgetAnalysis()` returns at the guard — run `testBudgetSystem()` instead (exercises `computeBudgetSignals_` and `computeLeadPace_` and logs the lead pace). Once re-enabled: run `runBudgetAnalysis()` manually, inspect the Slack proposal (lead-pace headline, CPL-sorted rows) and the `budget_queue` sheet, reject the proposal to avoid real Meta writes. For the strategic flow _(retired 2026-09-15)_: there is nothing to approve — dispatch `agent-portfolio-scaling.yml` and check that the Slack brief has two sections, names no paused campaign or retired vertical, and carries no approve/reject URL. To verify the guards, open an old `?action=approve_scaling&token=…` link (it must return "Strategic reallocation is retired." and post nothing to Slack) and run `executeStrategicChanges()` from the editor (it must log the retirement line, delete the five `SCALING_*` properties and return without a Meta call).
...
Every brief follows the lead-first Slack standard in §6.2 (headline leads / CPL, CPL sorts, CPL thresholds, no IC / ICP / CPICP / lead-subtype mention anywhere since 2026-09-14 — the 2026-09-10 allowance of one secondary `of which N reached an IC decision` line is withdrawn — and no stale optimizer copy). Each SKILL.md states it in its own output rules, and the matching `agent-*.yml` prompt repeats the same rules so the two instruction sources cannot disagree (on 2026-09-10 three of the prompts were still pre-pivot while their SKILL.md was not).

| Skill | Status | Purpose |
|---|---|---|
| `pipeline-health` | shipped 2026-05-03; five checks since 2026-09-09 | Checks: data freshness, Meta token, funnel conversions (quality tier first, subtypes trailing under `subtypes (reported only)`, subtype problems capped at WARN and phrased as tracking-config issues), dashboard endpoint (`?action=rollup`), snapshot volume (row counts + `N leads on $S spend`; optional zero-lead WARN floor). _(2026-09-14)_ `funnel_conversions` also returns `slack_status` / `slack_detail` covering the quality tier only, and that is what Slack sees — a subtype problem never produces a Slack line. Slack lines are `STATUS check_name: detail` (each check's `slack_*` pair when present), FAIL before WARN, silent on PASS. Deterministic — runs inside `daily-data.yml` with no LLM. |
| `daily-check` | shipped 2026-05-03 | `📊 Daily Lead Check` briefing: totals headline (leads · CPL · spend · prequal decisions; no IC parenthetical or `· IC n` suffix since 2026-09-14), pacing vs the live weekly target, portfolio sorted by CPL (no-lead campaigns last, `0 leads` explicit), top-3 winners by CPL (`≥5 leads + ≥1,000 impressions`), bleeders by `reason` (spend-without-leads → CPL ≥ 1.5× ad-set CPL → CTR fallback only without lead data), early fatigue flags, learning-phase ad sets, stale creatives. Writes to `daily_check_log` (legacy IC-named keys carrying lead values). |
...
| `portfolio-scaling` | shipped 2026-05-08; **diagnosis-only since 2026-09-15** | Tuesday brief, two sections and no proposal: 12-week portfolio headline (`total_leads / total_spend / cpl`; `median_cpl` dropped) + ACTIVE-only live-spend line, then per-vertical scaling labels CPL-ascending (`insufficient` rendered as a "too new to classify" one-liner), then audience actions derived from `new_audience_needed`. Renders `verticals` only — verticals with no ACTIVE campaign live under `inactive_verticals` and are never named, counted or alluded to. **Writes nothing:** the `scaling-queue-write` registration, the approve/reject links, the last-week evaluation and the `scaling-write` POST to `scaling_log` all retired with the pool (§3.7, §8.7); one script (`compute_scaling_profiles.py`) commits `data/derived/scaling_profiles.json` and that commit history is the classification record. |
...
| `check_data_freshness` / `check_meta_token` / `check_funnel_conversions` / `check_dashboard_endpoint` / `check_snapshot_volume` | `skills/pipeline-health/scripts/check_health.py` | Five health checks; each returns `{name, status, detail}`. `check_funnel_conversions` leads with the quality tier and trails `subtypes (reported only): …` most-recently-fired first (subtype problems ≤ WARN) and, since 2026-09-14, also returns `slack_status` / `slack_detail` — the quality tier only; a subtype problem leaves `slack_status` at PASS. `check_snapshot_volume` sums `leads` (field name from `conversions.primary.field`, `conversions` fallback) and `spend` from the newest `ad_insights.json` and appends `N leads on $S spend` to every detail; optional `zero_lead_spend_floor_usd` WARN. Standalone by design — does not import `scripts/lib`. |
| `print_terminal_summary` / `slack_status` / `slack_detail` / `slack_lines` / `post_to_slack` / `status_one_liner` | `skills/pipeline-health/scripts/report_health.py` | Deterministic formatter for `check_health.py` JSON (added 2026-06-23). Prints the terminal summary, composes + POSTs the WARN/FAIL Slack alert (lines `STATUS name: detail` so the check name is always the first token, FAIL before WARN, silent on full PASS / no webhook; since 2026-09-14 the `slack_status()` / `slack_detail()` helpers pick a check's Slack-facing pair when present, else `status` / `detail`, verbatim), and writes the issue-#48 one-liner (`PASS 5/0/0` shape). Replaced the daily `claude-code-action` invocation when pipeline-health was merged into `daily-data.yml`. |
...
| `is_active` / `lifetime_leads_per_campaign` | `skills/portfolio-scaling/scripts/compute_scaling_profiles.py` | ACTIVE test (shared with the retired `compute_reallocation.is_actionable_campaign`); lifetime leads per campaign (sum of `meta_conversions`) for `optimizer_eligible`, mirroring `LIFETIME_MIN_CONVERSIONS` via `OPTIMIZER_LIFETIME_MIN_LEADS`. Since 2026-09-15 the same ACTIVE test also decides the `verticals` / `inactive_verticals` split via each vertical's `active_campaign_count` (§10.1). |
| ~~`pool_skip_reasons`~~ | `skills/portfolio-scaling/scripts/compute_reallocation.py` | `paused_saturating_verticals` + `increase_skip_reason` so the brief could say why a side of the pool was empty. **Retired 2026-09-15** with the rest of the script — no workflow runs it and the brief has no pool section (§10.4). |

...
_Last updated: 2026-07-06 (Documentation accuracy pass — no functional/code changes in this update, only doc corrections. `apps-script/Code.js` grew from ~4,200 to ~6,493 lines over many prior PRs without this doc's line-count mentions or its ~100 `Code.js:NNN` cross-references ever being re-verified against the file; every reference in this document has now been re-checked against the current file and corrected (constants block is `Code.js:24-95`, not `14-52`; `generateNarrativeForWeek_` is at 1687 not 1373; `pushSnapshotToGitHub_` is at 6238 not ~4067; etc — see git diff for the full set). Two claims turned out to be factually wrong, not just stale: (1) §6.1 described a `buildAICommentary_()` function and an `aiSummarizeRow_()` function that do not exist — budget commentary is inline in `postBudgetProposalToSlack_()`, and the "5th Anthropic call site" this doc attributed to a portfolio-scaling confidence label is actually `testAnthropicConnection()`, a manual connectivity smoke test; the real scaling-brief confidence label has no LLM call at all (`getVerticalClassification_()` reads `scaling_profiles.json`). (2) §11.3/§11.4 claimed the fatigue signal's `min_days_active` floor is 3 days and configurable via a `snapshot_retention.rolling_window_days` benchmarks.json key that doesn't exist in the current schema — the actual value is 7 (`fatigue.min_days_active` in `benchmarks.json`), and the window is a hardcoded default in `compute_signals.py` overridable only via `--window-days`. CLAUDE.md's matching "≥ 3 days" signal-floor note and its Code.js line-count were corrected in the same pass. Prior: 2026-06-23 retired the standalone `agent-pipeline-health.yml` workflow and folded the pipeline-health check into `daily-data.yml` as two deterministic steps — `check_health.py` (unchanged) then the new `report_health.py` (terminal summary + Slack alert + issue-#48 one-liner). The check was always deterministic; running it via `claude-code-action` daily was a per-day Anthropic spend for pure JSON reformatting. The merge removes one scheduled workflow and one daily LLM invocation with zero loss of outcomes. Apps Script fallback `triggerAgentPipelineHealthIfNeeded` repointed to dispatch `daily-data.yml` (function name kept so the installed trigger binding stays valid). Also moved the two active scheduled crons off the top of the hour — `daily-data.yml` `0 12` → `37 12`, `agent-portfolio-scaling.yml` `30 13` → `43 13` — because GitHub's top-of-hour scheduled-run contention was delaying runs 1.8-5.1 h in this repo; uncommon minutes pull starts back toward schedule. Prior: 2026-06-10 PAUSED `agent-fatigue-monitor.yml`; three agent workflows remain paused — daily-check, creative-intelligence, and fatigue-monitor)_
...
- **Model:** Controlled by the `ANTHROPIC_MODEL` constant (Code.js:65). Currently set to `claude-opus-4-7`. All 5 production call sites reference the constant (a 6th call, in the manual-only `testAnthropicConnection()` diagnostic, also references it but isn't part of any scheduled or user-facing flow).
...
|---|---|---|
| `generateNarrativeForWeek_()` (Code.js:1687, call at 1871) | 1000 | Weekly Slack narrative in fixed format (OVERALL / SEGMENTS / WATCH / ACTION). On failure now inlines the error (HTTP code + body excerpt, exception message, or response-shape note) directly into the Slack fallback string instead of pointing readers at `intelligence_log` (improved 2026-05-11). |
| Budget commentary, inline in `postBudgetProposalToSlack_()` (Code.js:3632, call at 3719) | 800 | Commentary on proposed budget changes (SITUATION / CHANGES / WATCH). There is no separate `buildAICommentary_()` function — the prompt-building and Anthropic call are inline in `postBudgetProposalToSlack_()`. Bumped from 500 → 800 on 2026-05-09 after a CHANGES section truncated mid-word with 11 campaigns to summarize. |
| AI helper inline in `postDailyDigest()` (Code.js:2117, call at 2250) | 200 | Short "yesterday in one sentence" tail on the daily digest. `runDailyPipeline` calls `postDailyDigest()`; the AI call itself lives inside the latter, not literally inside `runDailyPipeline`. |
| `testAnthropicConnection()` diagnostic (Code.js:2704, call at 2710) | 20 | **Not part of the portfolio-scaling brief** — there is no `aiSummarizeRow_()` function. This is a manual-only connectivity smoke test ("Reply with only the word: connected"), not invoked by any scheduled pipeline. The portfolio-scaling brief's confidence label is derived from `scaling_profiles.json` classification data via `getVerticalClassification_()` (Code.js:3083, applied at ~3535) — no LLM call involved. |
| `handleChatRequest_()` (Code.js:5773, call at 5916) | 1500 | "Hive Mind" interactive chat with live data context |
...
| `postBudgetProposalToSlack_` | Code.js:3632 | Slack proposal with AI commentary (inline, no separate `buildAICommentary_` function). Sub-dollar "reductions" render with `→ (held flat)` instead of `↓ (-0.1%)`. |
...
| `applyBudgetQueueRows_` | Code.js:3920 | Shared per-row apply loop (optimizer + strategic). Both `executeBudgetChanges` and `executeStrategicChanges` delegate here with their own filter callback. |
...
|---|---|---|
| ~~Claude model hardcoded in 3 places~~ | ~~Code.js~~ | **Resolved 2026-04-22.** Extracted to `ANTHROPIC_MODEL` constant (Code.js:65). All 5 production call sites reference the constant. |
| Hybrid attribution math duplicated | Code.js: `buildWeeklyRollup` ~1489 and `computeBudgetSignals_` ~2808 | Risk of drift between `buildWeeklyRollup` and `computeBudgetSignals_` |
...
| `SYNC_WARNED_CAMPAIGNS` as pipe-delimited string | Code.js:~613 (inside `syncCampaignMappings_`) | Fragile if names contain pipes |
| Slack digest duplicates WoW/4wk metric math | Code.js: `postDailyDigest` ~2117-2205 | Shares logic with `generateNarrativeForWeek_` |
...
Computed by `scripts/compute_signals.py` from the most recent N snapshots. Default is 7 days, hardcoded in `compute_signals.py:main()` (with an explicit comment noting `rolling_window_days` is not a key in the current `benchmarks.json` schema); override per-run via `--window-days`.
...
- `actionable: false` is always set if days_active < 7 or impressions < 1,000 or ad set in learning phase

...
_Last updated: 2026-07-10 (Documentation accuracy pass — no functional code changed since 2026-06-23. Corrected drift that had accumulated in this file: (1) `Code.js` line-count figure was `~4,200`, actual is `~6,493` — every line-number citation below was stale as a result, some by 2,000+ lines; all citations in §4.1, §5-9, §10.2-10.5 have been re-verified against the current file. (2) §6.1's "five call sites" table named a function, `aiSummarizeRow_()`, that does not exist in `Code.js` and never has — the real 5th `ANTHROPIC_MODEL` call site is the diagnostic `testAnthropicConnection()`. (3) A second fictional name, `buildAICommentary_()`, was also removed — the budget-commentary Anthropic call is inline code inside `postBudgetProposalToSlack_()`, not a separate function. (4) §10.1's descriptions of `agent-daily-check.yml`, `agent-fatigue-monitor.yml`, and `agent-creative-intelligence.yml` said "active cron" — all three were paused between 2026-06-08 and 2026-06-10 (correctly reflected in CLAUDE.md, never updated here). Prior: 2026-06-23 consolidated pipeline-health into daily-data.yml + de-peaked the two active crons)_
...
│   ├── preview_dataset.py   # Deterministic (no-LLM) creative preview brief
│   ├── run_daily.sh         # Orchestrator (fetch → compute)
│   └── lib/                 # Shared client: meta.py, exec_api.py, io.py, text_features.py
├── skills/                  # NEW (2026-05-02) Agent skill definitions — 6 skills
...
│   ├── daily-data.yml               # Ad-level data pull, daily cron (active) + pipeline-health steps
│   ├── agent-daily-check.yml        # daily-check skill (PAUSED 2026-06-08, workflow_dispatch only)
│   ├── agent-fatigue-monitor.yml    # fatigue-monitor skill (PAUSED 2026-06-10, workflow_dispatch only)
│   ├── agent-creative-intelligence.yml  # creative-intelligence skill (PAUSED 2026-06-08, workflow_dispatch only)
│   ├── agent-creative-preview.yml   # $0 no-LLM preview path (workflow_dispatch only)
│   ├── agent-ad-copy-generator.yml  # ad-copy-generator skill (workflow_dispatch only)
│   └── agent-portfolio-scaling.yml  # portfolio-scaling skill, weekly Tuesday cron (active)
│   # (agent-pipeline-health.yml existed 2026-05-03–2026-06-23; retired, merged into daily-data.yml)
...
### 4.1 Hardcoded constants (Code.js:24-65)
...
**Five call sites** (none are separately-named "AI commentary" functions except where noted — three are inline `UrlFetchApp.fetch` calls nested inside a larger function):

| Caller | `max_tokens` | System prompt purpose |
|---|---|---|
| Inline call inside `generateNarrativeForWeek_()` (Code.js:1871, function starts 1687) | 1000 | Weekly Slack narrative in fixed format (OVERALL / SEGMENTS / WATCH / ACTION). On failure now inlines the error (HTTP code + body excerpt, exception message, or response-shape note) directly into the Slack fallback string instead of pointing readers at `intelligence_log` (improved 2026-05-11). |
| Inline commentary call inside `postBudgetProposalToSlack_()` (Code.js:3727, function starts 3632) | 800 | Commentary on proposed budget changes (SITUATION / CHANGES / WATCH). Bumped from 500 → 800 on 2026-05-09 after a CHANGES section truncated mid-word with 11 campaigns to summarize. Not a separate function — earlier revisions of this doc called it `buildAICommentary_()`, which does not exist in `Code.js`. |
| Inline call inside `postDailyDigest()` (Code.js:2250, function starts 2117) | 200 | Short "yesterday in one sentence" tail on the daily digest. Earlier revisions of this doc attributed this to `runDailyPipeline`; `runDailyPipeline` (Code.js:2310) only orchestrates the pipeline stages and doesn't call Anthropic directly — `postDailyDigest` is one of the functions it calls. |
| `testAnthropicConnection()` (Code.js:2718, function starts 2704) | 20 | Diagnostic connectivity ping ("Reply with only the word: connected"), not part of any user-facing brief. Earlier revisions of this doc invented a 5th call site, `aiSummarizeRow_()` for a "portfolio-scaling brief" — that function does not exist anywhere in `Code.js`; the portfolio-scaling Slack brief is composed by `claude-code-action` reading committed JSON files (see `agent-portfolio-scaling.yml`), not by an in-script Anthropic call. |
| Inline call inside `handleChatRequest_()` (Code.js:5916, function starts 5773) | 1500 | "Hive Mind" interactive chat with live data context |
...
```js
var scalingProfiles = loadScalingProfiles_();  // raw GitHub fetch + 1hr cache — Code.js:2961
var lockoutSet      = getScalingLockoutSet_();  // {} when SCALING_LOCKOUT_UNTIL absent or past — Code.js:3011
var prevTue         = previousTuesdayUTC_();    // headroom window start — Code.js:3032
```
...
- Triggers: `workflow_dispatch` only. The `schedule:` block and cron entry (`30 12 * * *`, 8:30 AM ET / UTC 12:30) are commented out in the YAML, not deleted, so the original timing is preserved for whoever unpauses it. The Apps Script fallback (`triggerAgentDailyCheckIfNeeded`) has a matching early-return guard.
...
- Same template.
- Triggers: `workflow_dispatch` only. The `schedule:` block and cron entry (`30 13 * * 1,4`, Mon + Thu 9:30 AM ET / UTC 13:30 — twice-weekly because fatigue moves slowly and daily would over-query Meta) are commented out, not deleted. The Apps Script fallback (`triggerAgentFatigueMonitorIfNeeded`) has a matching early-return guard.
...
- Triggers: `workflow_dispatch` only. The `schedule:` block and cron entry (`0 14 * * 1`, Mon 10 AM ET / 9 AM EST — weekly cadence matches the corpus-aggregation attribution model) are commented out, not deleted, so Tyler can still run it on demand to refresh the cache. The Apps Script fallback (`triggerAgentCreativeIntelligenceIfNeeded`) has a matching early-return guard. Reason for the pause: stop the weekly Slack brief and the recurring Anthropic categorization spend.
...
**`agent-portfolio-scaling.yml`** _(added 2026-05-08, active)_

- Triggers: `workflow_dispatch` + active weekly cron `43 13 * * 2` (Tuesdays ~9:43 AM ET / UTC-4 EDT, ~8:43 AM ET during EST). Minute moved off `:30` on 2026-06-23 to dodge GitHub's scheduled-run queue contention, same reasoning as the `daily-data.yml` minute change.
- Follows the scripts-before-`claude-code-action` pattern (§10.1.0.5): two Python steps (`compute_scaling_profiles.py` → `compute_reallocation.py`) run as ordinary workflow steps and commit `data/derived/scaling_profiles.json` + `reallocation.json` to main, THEN `claude-code-action@v1` reads that committed JSON, registers the proposal via `/exec?action=scaling-queue-write` to get an approval token, composes the four-section Slack brief, and posts it.
- Permissions: `contents: write` (commit derived JSON) + `id-token: write` (OIDC for `claude-code-action`) + `issues: write` (status comment to issue #48).
- Execution side runs separately: `executeStrategicChanges()` in `Code.js` fires daily at 3 AM (not just Wednesdays) so a no-op check is cheap and a late approval still executes on the next cycle — see §7.1 and §8.7.
...
| `runDailyPipeline` | Code.js:2310 | Scheduled daily-pipeline orchestrator (calls the above in sequence) |
...
| Hybrid attribution math duplicated | Code.js: `buildWeeklyRollup` ~1485-1490 and `computeBudgetSignals_` ~2877-2893 | Risk of drift between `buildWeeklyRollup` and `computeBudgetSignals_` |
...
| `SYNC_WARNED_CAMPAIGNS` as pipe-delimited string | Code.js: `syncCampaignMappings_` ~613-663 | Fragile if names contain pipes |
| Slack digest duplicates WoW/4wk metric math | Code.js: `postWeeklyNarrativeToSlack_` ~2044 and `postDailyDigest` ~2192 | Two separate WoW computations for the same underlying metric |
...
| `daily-data.yml` backfill DETAIL line reads the wrong manifest | `.github/workflows/daily-data.yml` ~193 | In backfill mode, `DATE_LABEL` defaults to "yesterday UTC" (`SNAPSHOT_DATE_INPUT` is unset when the run uses `start_date`/`end_date` instead) so the manifest path resolved is yesterday's manifest, not the backfill range. HEADER is correct; DETAIL counts mismatch. Stitch a multi-date summary or skip DETAIL on backfill. Still open — confirmed present as of 2026-07-10. |
...
|---|---|---|
| `pipeline-health` | shipped 2026-05-03; autonomous run merged into `daily-data.yml` 2026-06-23 (no longer a separate scheduled workflow or LLM call) | Four checks: data freshness, Meta token, IC conversion event, dashboard endpoint. Slack-silent on PASS. |
| `daily-check` | shipped 2026-05-03; autonomous workflow PAUSED 2026-06-08 (manual `workflow_dispatch` still works) | Morning briefing: pacing vs weekly target, portfolio CPICP rankings, top-3 winners + bleeders, early fatigue flags, learning-phase ad sets, stale creatives. Writes to `daily_check_log`. |
| `fatigue-monitor` | shipped 2026-05-03; autonomous workflow PAUSED 2026-06-10 (manual `workflow_dispatch` still works) | Three-script pipeline: 14-day fetch, baseline computation (Path A in-range / B historical-batched / C estimated), classification across 5 severity classes with budget-queue conflict cross-reference. Writes to `fatigue_log`. |
| `creative-intelligence` | shipped 2026-05-05; autonomous workflow PAUSED 2026-06-08 (manual `workflow_dispatch` still works, useful for refreshing the cache) | Weekly Monday brief on winning creative copy/visual patterns via corpus-level text aggregation (see `docs/CREATIVE_INTELLIGENCE_DESIGN.md`). Writes to `creative_intelligence_log`. |
| `ad-copy-generator` | shipped 2026-05-05; `workflow_dispatch`-only by design (drafts always need human review) | Drafts new ad-copy variants for a target vertical from the Creative Intelligence dataset. Never auto-published. |
| `portfolio-scaling` | shipped 2026-05-08; autonomous, active weekly Tuesday cron | Structural diagnosis layer (elasticity, CPL degradation, frequency/CPM trends) on top of the daily budget optimizer, with pool-based reallocation. See §8.7. |

`compute_signals.py`'s `data/derived/` outputs are an audit trail rather than the canonical signal source — the skills above compute their own canonical signals live from Meta.
...
| `scaling_log` | `portfolio-scaling` skill via `?action=scaling-write` (auto-created in `handleScalingWrite_`, Code.js:5368) | See §3.7 for the full 16-column schema (classification, confidence, elasticity_r, cpicp, frequency/CPM trend, etc.) |

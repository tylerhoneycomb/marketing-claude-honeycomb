...
| `BUDGET_RANK_TIERS` PROPS grows unboundedly | `Code.js: computeRecommendations_` ~3225 | When a campaign disappears from `rankable` (paused, archived), its tier entry persists in the JSON. PROPS has a 9 KB string limit; with ~11 active campaigns the JSON is ~300 bytes, so ~300 campaign churn-cycles before approaching the limit. Negligible in practice but pruning entries that didn't appear in the current run would be tidy. |

...
_Last updated: 2026-06-12 (Documentation sync: corrected skill count from three to six; marked daily-check, fatigue-monitor, and creative-intelligence workflows as PAUSED in the autonomous-skill-execution section; promoted ad-level pipeline autonomy from "Operational gaps" to "What's working well"; added prompt-caching cost note and strategic-lockout-window detail; updated summary paragraph)_
...
Skills are self-contained packages under `skills/<name>/` with a `SKILL.md` operating manual and Python scripts that handle Meta API calls and computation. Claude Code reads them at session start and runs the scripts via bash. Six skills are defined (three are currently paused at Tyler's request to reduce noise — they can be re-enabled by uncommenting the cron and restoring the Apps Script fallback):
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** at Tyler's request (daily Slack briefing suspended). GitHub cron commented out; Apps Script fallback `triggerAgentDailyCheckIfNeeded` early-returns. Manual `workflow_dispatch` still available. Was: daily cron at 8:30 AM ET (UTC 12:30).
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** at Tyler's request (Mon/Thu Slack brief suspended). GitHub cron commented out; Apps Script fallback `triggerAgentFatigueMonitorIfNeeded` early-returns. Manual `workflow_dispatch` still available. Was: Mon + Thu 9:30 AM ET (UTC 13:30) — fatigue moves slowly, daily would over-query Meta.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** at Tyler's request (weekly Slack brief and recurring Anthropic categorization spend suspended). GitHub cron commented out; Apps Script fallback `triggerAgentCreativeIntelligenceIfNeeded` early-returns. Manual `workflow_dispatch` still available and useful for on-demand cache refresh. Was: weekly cron at Monday 10 AM ET (UTC 14:00). Weekly cadence matches the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:
...
- **Ad-level pipeline runs fully autonomously.** `daily-data.yml` fetches ad-set and ad-level snapshots from Meta and commits them to `data/snapshots/` at 8 AM ET every morning with no manual intervention. Manual `workflow_dispatch` with `start_date`/`end_date` inputs handles backfills.
- **Prompt caching on Creative Intelligence categorizer cuts cost ~10×.** The 5,000-token system message is cached across all ~526 Anthropic calls in a single run via `cache_control: ephemeral`. Cost dropped from ~$5 to ~$1–2/run; rate-limit failures dropped from 18% to ~1%. Benefit preserved even when the workflow is paused — it applies immediately on the next manual dispatch.
- **Dual-scheduler reliability.** Each active agent workflow has two independent firing paths: a GitHub Actions cron (primary) and an Apps Script time-based trigger (fallback, fires ~3 hours later if GitHub missed or failed). Portfolio-scaling and pipeline-health both benefit from this; paused skills have their fallback triggers early-returned but not removed.
...
This is a mature, working automation platform. The core data pipeline runs daily without intervention, the budget optimizer has two-step human-in-the-loop safeguards (Slack confirmation page defeats link-unfurling bots), and campaign renames in Meta are handled automatically without manual mapping cleanup. The Q1 audit's 6 data-quality issues are all resolved, IC conversion tracking has been restored (now using the cleaner "Investment Crowdfunding Prequal Decision" event), and the AI layer runs on Claude Opus 4.7 via a single configurable constant. Six agent skills have shipped — pipeline-health, daily-check, fatigue-monitor, creative-intelligence, ad-copy-generator, and portfolio-scaling — with daily-check, fatigue-monitor, and creative-intelligence paused at Tyler's request since 2026-06-08/10 to reduce Slack noise. The pipeline-health and portfolio-scaling workflows continue running on their active cron schedules, and all six skills remain available for manual `workflow_dispatch` dispatch. The strategic reallocation (portfolio-scaling) locks affected campaigns from Wednesday through Monday after each Tuesday execution so the daily optimizer doesn't compound on top of a fresh structural move. The biggest remaining ROI improvements are around observability: proactive alerts on pipeline failures, attribution-quality drops, and token expiration. The system does not currently tell you when it's broken — you have to notice.

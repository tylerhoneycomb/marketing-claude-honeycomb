...
_Last updated: 2026-06-30 (Documentation accuracy pass — no functional changes. Fixed a stale Code.js line count (~4,200 → ~6,500 actual), corrected the "three skills are scoped" / 6-vs-7-item checklist / 4-vs-5 compliance-category undercounts, added the previously-undocumented dashboard password gate (client-side only, added 2026-05-26), and added Known Risk #5: the Anthropic account behind the GitHub `ANTHROPIC_API_KEY` secret ran out of credit balance mid-run on 2026-06-16 and 2026-06-30, silently dropping the weekly portfolio-scaling Slack brief both times — safe (the proposal just expires unapproved) but a real recurring gap worth fixing. Prior: 2026-06-23 consolidated the daily pipeline-health check into the daily-data workflow; daily-check, creative-intelligence, and fatigue-monitor remain paused)_
...
- Pulls 12 weeks of trailing data and classifies each business vertical (breweries, bakeries, wineries, etc.) as **scalable**, **stable**, **saturating**, or **over-invested** based on how its cost-per-lead behaves as spend goes up (verticals with too little data get **insufficient** instead and are left out of the brief). Verticals where frequency AND CPM are both rising over the last 4 weeks get an additional **new-audience-needed** flag — a warning that more budget won't fix the problem, the audience needs to expand.
...
Skills are self-contained packages under `skills/<name>/` with a `SKILL.md` operating manual and Python scripts that handle Meta API calls and computation. Claude Code reads them at session start and runs the scripts via bash. Six skills are scoped:
...
- **Web App is publicly accessible (ANYONE_ANONYMOUS).** The `/exec` URL has no auth. Anyone who knows the URL can hit the dashboard API directly. Dashboard data isn't super-sensitive (campaign metrics) but it's worth knowing.
- **The dashboard UI has a password gate, but it's cosmetic.** _(added 2026-05-26)_ `webapp/index.html` now shows a password prompt before rendering the dashboard (checks a SHA-256 hash client-side, remembers you for the browser session via `sessionStorage`). This stops casual browsing of the GitHub Pages URL, but it does NOT protect the underlying data — the `/exec` API above has no auth and can still be queried directly with curl/fetch, bypassing the gate entirely. Treat it as a "keep it out of search engines and screen-shares" speed bump, not real access control.
...
5. **Anthropic billing has run dry mid-run twice in three weeks, silently dropping the weekly strategic-reallocation brief.** _(found 2026-06-30)_ The Tuesday `agent-portfolio-scaling.yml` run failed with `"Credit balance is too low"` on 2026-06-16 and again on 2026-06-30 (succeeded on 2026-06-23). The data math always completes fine — only the brief-composition step, which is the only thing that calls Anthropic, dies partway through. On 2026-06-30 it died *after* registering that week's strategic reallocation proposal in `budget_queue` but *before* posting the Slack brief, so a real pending proposal sat unapproved with no notification. It's not unsafe — the human-approval safeguard still holds, and the proposal automatically expires unactioned at the next 3 AM execution run rather than applying itself — but it means that week's reallocation opportunity is silently lost, and nobody finds out unless they happen to check the `pipeline-health`/agent-loop tracking issue (#48) and notice a `(no brief status)` comment instead of the usual numbers. **Action needed:** check the Anthropic account/billing behind the `ANTHROPIC_API_KEY` GitHub Secret and raise the balance or auto-reload threshold — this is a billing/ops fix, not a code fix.
...
- Top up / raise the auto-reload threshold on the Anthropic account behind the `ANTHROPIC_API_KEY` GitHub Secret — it ran dry mid-run on 2026-06-16 and 2026-06-30, silently dropping the weekly portfolio-scaling brief both times. See Known Risks #5.

...
_Last updated: 2026-06-15 (Documentation audit pass — updated autonomous-skill section to reflect three paused workflows: `agent-daily-check.yml` paused 2026-06-08, `agent-creative-intelligence.yml` paused 2026-06-08, `agent-fatigue-monitor.yml` paused 2026-06-10. No code or pipeline changes; only docs brought in sync with CLAUDE.md)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** at Tyler's request. GitHub cron (8:30 AM ET / UTC 12:30) commented out; Apps Script fallback `triggerAgentDailyCheckIfNeeded` early-returns with a PAUSED log line. Manual `workflow_dispatch` still works.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** at Tyler's request. GitHub cron (Mon + Thu 9:30 AM ET / UTC 13:30) commented out; Apps Script fallback `triggerAgentFatigueMonitorIfNeeded` early-returns with a PAUSED log line. Manual `workflow_dispatch` still works.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** at Tyler's request. GitHub cron (Monday 10 AM ET / UTC 14:00) commented out; Apps Script fallback `triggerAgentCreativeIntelligenceIfNeeded` early-returns. Manual `workflow_dispatch` still works (useful for refreshing the cache on demand). Weekly cadence matches the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

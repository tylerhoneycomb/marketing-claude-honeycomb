...
_Last updated: 2026-06-26 (Documentation sync: corrected workflow status for `agent-daily-check`, `agent-fatigue-monitor`, and `agent-creative-intelligence` — all three were paused in early June 2026 but STATE_REPORT still described their crons as "active." No code changed; docs only. Prior: 2026-06-23 — consolidated pipeline-health into daily-data, retired `agent-pipeline-health.yml`, de-peaked both active crons; 2026-06-10 PAUSED `agent-fatigue-monitor.yml`; 2026-06-08 PAUSED `agent-daily-check.yml` and `agent-creative-intelligence.yml`)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-08**)_ — Tyler asked to stop the daily Slack briefing. The scheduled cron (8:30 AM ET / UTC 12:30) and the Apps Script fallback (`triggerAgentDailyCheckIfNeeded`) are both commented out / early-returned. Manual `workflow_dispatch` still works. Cron timing is preserved as a comment in the YAML so it's obvious how to unpause.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-10**)_ — Tyler asked to stop the Mon/Thu Slack brief. The twice-weekly cron (Mon + Thu 9:30 AM ET / UTC 13:30) and the Apps Script fallback (`triggerAgentFatigueMonitorIfNeeded`) are both commented out / early-returned. Manual `workflow_dispatch` still works. Twice-weekly was the right cadence (fatigue moves slowly; daily would over-query Meta); timing preserved in the YAML as a comment.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05, **PAUSED 2026-06-08**)_ — Tyler asked to stop the weekly Slack brief and the recurring Anthropic categorization spend. The weekly cron (Monday 10 AM ET / UTC 14:00) and the Apps Script fallback (`triggerAgentCreativeIntelligenceIfNeeded`) are both commented out / early-returned. Manual `workflow_dispatch` still works (useful for refreshing the cache on demand without paying the autonomous Anthropic cost). Weekly cadence matched the corpus-aggregation attribution model — variant-level performance signals shift over weeks, not days. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

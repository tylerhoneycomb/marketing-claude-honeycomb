...
_Last updated: 2026-06-16 (Documentation sync: corrected agent workflow body text — daily-check, creative-intelligence (paused 2026-06-08), and fatigue-monitor (paused 2026-06-10) were still described as "active" in the autonomous-skill section. Updated workflow-pattern grouping to seven workflows, corrected fallback-function count. No code changes in this commit.)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** (Tyler disabled daily Slack briefings; Apps Script fallback `triggerAgentDailyCheckIfNeeded` also early-returns). `workflow_dispatch` still works; cron is commented out (was 8:30 AM ET / UTC 12:30).
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** (Tyler disabled Mon/Thu Slack briefings; Apps Script fallback `triggerAgentFatigueMonitorIfNeeded` also early-returns). `workflow_dispatch` still works; cron is commented out (was Mon + Thu 9:30 AM ET / UTC 13:30 — twice-weekly because fatigue moves slowly and daily would over-query Meta).
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** (Tyler disabled the weekly Slack brief and recurring Anthropic categorization spend; Apps Script fallback `triggerAgentCreativeIntelligenceIfNeeded` also early-returns). `workflow_dispatch` still works. Weekly cron was Monday 10 AM ET (UTC 14:00); cadence matched the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:
...
The seven workflows fall into two patterns:
...
- **`agent-creative-intelligence` / `agent-creative-preview` / `agent-ad-copy-generator` / `agent-portfolio-scaling`** — established the new pattern: scripts run as ordinary workflow steps BEFORE `claude-code-action`; cache/artifact commits happen BEFORE the action runs too (or instead of it for the preview/drafter, neither of which uses claude-code-action at all). New skills with Anthropic-subprocess or commit-back needs should follow this pattern.

### Cron fallback via Apps Script (added 2026-05-03)

GitHub Actions cron is best-effort — runs can be delayed, occasionally skipped, and **silently disabled after 60 days of repo inactivity**. To make scheduled runs more reliable, Apps Script time-based triggers (running on Google's cron infrastructure) act as a fallback. Five functions in `apps-script/Code.js` (`triggerAgent*IfNeeded`) fire ~3 hours after each GitHub cron is supposed to run (or on their designated day), check the GitHub API for a recent successful or in-progress run, and dispatch via `workflow_dispatch` only if none exists. If GitHub fired on time, Apps Script skips. If GitHub missed, the fallback picks it up. After running `createAllTriggers()` from the Apps Script editor, the system has two independent schedulers covering each active workflow.

...
- **AI upgraded to Claude Opus 4.7.** All Anthropic calls made directly by the Apps Script brain (narrative, chat, budget commentary, daily digest, plus a diagnostic connectivity ping) use a single `ANTHROPIC_MODEL` constant — future upgrades are one line. (Weekly Slack posts the already-generated narrative; it's not a separate AI call.)
...
- ~~**Hardcoded Claude model in 3 places.**~~ _Resolved 2026-04-22._ Extracted to `ANTHROPIC_MODEL` constant (Code.js:65). Upgraded to Opus 4.7. All 5 Anthropic call sites in Code.js reference the constant.

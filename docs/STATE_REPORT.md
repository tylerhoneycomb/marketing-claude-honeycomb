...
_Last updated: 2026-06-11 (doc sync audit: corrected Code.js line count to ~6,500; updated agent workflow descriptions to accurately reflect PAUSED status for daily-check, fatigue-monitor, and creative-intelligence)_
...
- **`agent-pipeline-health.yml`** _(shipped 2026-05-03)_ — **ACTIVE.** Daily cron at 9 AM ET (UTC 13:00). v1 of the autonomous-agent pattern.
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08.** `workflow_dispatch` only (cron commented out; was: daily 8:30 AM ET / UTC 12:30).
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10.** `workflow_dispatch` only (cron commented out; was: Mon + Thu 9:30 AM ET / UTC 13:30 — fatigue moves slowly, daily would over-query Meta).
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08.** `workflow_dispatch` only (cron commented out; was: Monday 10 AM ET / UTC 14:00). Weekly cadence matched the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

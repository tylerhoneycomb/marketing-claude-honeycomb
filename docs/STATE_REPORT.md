...
_Last updated: 2026-06-25 (Documentation sync: corrected paused-workflow status in body text (daily-check, fatigue-monitor, creative-intelligence were described as "active" despite being paused 2026-06-08/10); updated skill count from 3 to 6; updated pipeline-failure alerting limitation to reflect pipeline-health integration. Prior: 2026-06-23 Consolidated pipeline-health into daily-data.yml, retired agent-pipeline-health.yml, de-peaked cron times. 2026-06-10 PAUSED agent-fatigue-monitor.yml; daily-check, creative-intelligence, and fatigue-monitor remain paused)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** (Tyler asked to stop the daily Slack briefing). Cron `30 12 * * *` (8:30 AM ET) and the Apps Script fallback are both commented out. Manual `workflow_dispatch` still works.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** (Tyler asked to stop the Mon/Thu Slack brief). Cron `30 13 * * 1,4` (Mon + Thu 9:30 AM ET) and the Apps Script fallback are both commented out. Manual `workflow_dispatch` still works.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** (Tyler asked to stop the weekly Slack brief and the recurring Anthropic categorization spend). Cron `0 14 * * 1` (Mon 10 AM ET) and the Apps Script fallback are both commented out. Manual `workflow_dispatch` still works (useful for refreshing the cache on demand). Weekly cadence matches the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:
...
- **Automated pipeline-health checks.** Four daily checks (data freshness, Meta token validity, IC conversion event existence, dashboard endpoint health) run as deterministic Python steps at the end of `daily-data.yml` (merged 2026-06-23). Posts a Slack alert on WARN/FAIL. No LLM required, no separate workflow, no extra Anthropic cost.
...
- **Limited alerting on pipeline failures.** The pipeline-health check (merged into `daily-data.yml` 2026-06-23) now posts a Slack alert on WARN/FAIL for data freshness gaps, Meta token expiry warnings, and dashboard endpoint health — so some failure modes are caught proactively. However, if the GitHub Action itself fails entirely (e.g. a Python crash before the health-check step), the alert doesn't fire; you still rely on GitHub's own email notifications or noticing the digest didn't arrive. Full "did my workflow run?" alerting would require a separate heartbeat monitor.
...
- ~~Add a pipeline-health check: if the daily digest hasn't posted by 8 AM, something's broken — alert.~~ **Done 2026-06-23** — four daily checks now run as deterministic steps inside `daily-data.yml`; Slack alert on WARN/FAIL.

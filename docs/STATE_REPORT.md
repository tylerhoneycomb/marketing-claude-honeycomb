...
_Last updated: 2026-07-02 (Documentation accuracy pass — no functional changes. Corrected the `apps-script/Code.js` line count, which had been stale at "~4,200 lines" since at least 2026-06-23 while the file had actually grown to ~6,500; fixed the `agent-daily-check.yml` / `agent-fatigue-monitor.yml` / `agent-creative-intelligence.yml` entries below, which still described their crons as "active" even though all three were paused 2026-06-08/06-10 — see [TECHNICAL_REFERENCE.md](./TECHNICAL_REFERENCE.md) for the equivalent fixes plus a full re-verification of every `Code.js` function line reference. Prior: 2026-06-23 consolidated the daily pipeline-health check into the daily-data workflow — see git history for that entry)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** (Tyler asked to stop the daily Slack briefing). Cron was `30 12 * * *` (8:30 AM ET / UTC 12:30); the schedule block is commented out in the workflow file, preserved for easy re-enable. Manual `workflow_dispatch` still works.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** (Tyler asked to stop the Mon/Thu Slack brief). Cron was `30 13 * * 1,4` (Mon + Thu 9:30 AM ET / UTC 13:30) — fatigue moves slowly, daily would over-query Meta; the schedule block is commented out, preserved for easy re-enable. Manual `workflow_dispatch` still works.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** (Tyler asked to stop the weekly Slack brief and the recurring Anthropic categorization spend). Cron was `0 14 * * 1` (Monday 10 AM ET / UTC 14:00) — weekly cadence matched the corpus-aggregation attribution model; the schedule block is commented out, preserved for easy re-enable. Manual `workflow_dispatch` still works (useful for refreshing the cache on demand). Three production runs on 2026-05-05 (while the cron was still active) surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

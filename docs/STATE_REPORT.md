...
_Last updated: 2026-06-24 (Documentation audit: corrected Code.js line count (~4,200 → ~6,500), marked paused workflows as PAUSED in the autonomous-skill section, expanded repo structure tree, updated skills table. No code changes. Prior: 2026-06-23 Consolidated pipeline-health check into daily-data.yml, nudged active crons off top-of-hour. 2026-06-10 PAUSED `agent-fatigue-monitor.yml`; daily-check, creative-intelligence, and fatigue-monitor remain paused)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-08**)_ — cron was daily 8:30 AM ET (UTC 12:30); now manual `workflow_dispatch` only. The Apps Script fallback (`triggerAgentDailyCheckIfNeeded`) is also early-returned. Tyler asked to stop the daily Slack briefing; re-enable by uncommenting the `schedule:` block and removing the early-return.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-10**)_ — cron was twice-weekly Mon + Thu 9:30 AM ET (UTC 13:30); now manual `workflow_dispatch` only. Fatigue moves slowly, so daily would over-query Meta; the original twice-weekly cadence is preserved as a comment in the YAML.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, **PAUSED 2026-06-08**)_ — cron was weekly Monday 10 AM ET (UTC 14:00); now manual `workflow_dispatch` only. Tyler asked to stop the recurring Anthropic categorization spend; manual dispatch still works for on-demand refresh. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

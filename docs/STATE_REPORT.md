...
_Last updated: 2026-06-04 (docs audit: corrected Code.js line count, "Three skills are scoped" intro, "five workflows" pattern label; expanded repo structure and skill table in TECHNICAL_REFERENCE.md to include ad-copy-generator and portfolio-scaling)_
...
Skills are self-contained packages under `skills/<name>/` with a `SKILL.md` operating manual and Python scripts that handle Meta API calls and computation. Claude Code reads them at session start and runs the scripts via bash. Six skills are active:
...
- **`agent-creative-intelligence` / `agent-creative-preview` / `agent-ad-copy-generator` / `agent-portfolio-scaling`** — follow the new pattern: Python scripts run as ordinary workflow steps and cache/artifact commits happen BEFORE `claude-code-action` (or instead of it). New skills with Anthropic-subprocess calls or commit-back needs should follow this pattern.

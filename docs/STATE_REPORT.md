...
_Last updated: 2026-06-22 (Documentation review: corrected autonomous-skill-execution section to reflect that `agent-daily-check`, `agent-creative-intelligence`, and `agent-fatigue-monitor` are paused (as of 2026-06-08 and 2026-06-10 respectively); updated workflow count from five to seven; updated skill count from three to six; no functional changes to the pipeline)_
...
Skills are self-contained packages under `skills/<name>/` with a `SKILL.md` operating manual and Python scripts that handle Meta API calls and computation. Claude Code reads them at session start and runs the scripts via bash. Six skills are live:
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-08** at Tyler's request. Cron was daily 8:30 AM ET (UTC 12:30); manual `workflow_dispatch` still works.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03)_ — **PAUSED 2026-06-10** at Tyler's request. Cron was Mon + Thu 9:30 AM ET (UTC 13:30); manual `workflow_dispatch` still works. Fatigue moves slowly; daily cadence would over-query Meta.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05)_ — **PAUSED 2026-06-08** at Tyler's request. Cron was weekly Monday 10 AM ET (UTC 14:00); manual `workflow_dispatch` still works. Weekly cadence matches the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

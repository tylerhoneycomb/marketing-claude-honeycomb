...
_Last updated: 2026-05-19 (PRs #79–80, #82: categorizer retry hardened, `compute_signals` gains `below_floor` severity bucket, `fetch_ad_data` refuses to silently overwrite snapshots, backfill manifest counting fixed, DST-drift caveat centralized in CLAUDE.md, whitelist class-of-bug documented)_
...
- **creative-intelligence** _(shipped 2026-05-05)_ — weekly Monday brief on what creative copy and visual patterns are winning across the portfolio. Tells Tyler what to write next by quoting actual winning copy alongside its real numbers (CPICP, IC count, ad count) and structural fingerprint (length, opening word, syntactic markers). The attribution model is corpus-level text aggregation: when the same body text appears across many ads, sum spend + IC across all of them to produce a meaningful per-variant CPICP. Three rounds of Meta API investigation proved that asset-level breakdown insights — the original spec's spine — won't return reliable per-variant conversion data for Honeycomb's `asset_feed_spec` ad mix; the design pivot is captured in [docs/CREATIVE_INTELLIGENCE_DESIGN.md](./CREATIVE_INTELLIGENCE_DESIGN.md). Two-script pipeline: `categorize_creative.py` (Anthropic API once per unique variant text + image, hash-deduped; ~$1-2 per run with prompt-cached system message; retry logic hardened in PR #79 to 5 attempts with exponential backoff + Retry-After parsing) and `build_creative_dataset.py` (joins snapshots + creative cache + categorizations, downloads full-size images via `/adimages` resolution, finds same-image-different-body side-by-side pairs). Writes per-vertical rollups to a new `creative_intelligence_log` Sheet tab. SKILL.md output rules require briefs that quote actual copy + cite real numbers + honor confidence labels — never recommend categories.
...
**Signal precision improvement (PR #80, 2026-05-11):** `compute_signals.py` now distinguishes a fourth fatigue severity bucket — `below_floor` — for ads that show fatigue flags but haven't yet crossed the minimum data threshold (< 3 days active or < 1,000 impressions). Previously these were grouped with `ok` (no signals at all), making it impossible to tell whether a young ad was clean or just not yet measurable. The change is backward-compatible: existing consumers that only check `critical`/`warning`/`ok` are unaffected.

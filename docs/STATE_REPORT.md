...
_Last updated: 2026-07-03 (Documentation accuracy pass — no functional changes. Fixed several places where this report had drifted from the actual code/config: the "Autonomous skill execution" section still described `daily-check`, `fatigue-monitor`, and `creative-intelligence` as running on active cron schedules when all three have been paused since June (their `schedule:` blocks and Apps Script fallbacks are commented out / early-return-guarded — this doc's own changelog already said so, the body text just hadn't caught up); the skills intro said "Three skills are scoped" while six have shipped; the Apps Script line-count estimate (~4,200 lines) was stale — the file is now ~6,500 lines; and the campaign-mapping figures in "Operational gaps" are now flagged as sourced from an audit-snapshot export that's over two months stale. Prior: 2026-06-23 consolidated pipeline-health into daily-data workflow; 2026-06-10 PAUSED `agent-fatigue-monitor.yml`; daily-check, creative-intelligence, and fatigue-monitor remain paused)_
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03, **paused 2026-06-08**)_ — Slack briefing disabled at Tyler's request. Cron (was daily 8:30 AM ET / UTC 12:30) is commented out; `workflow_dispatch` still works. Apps Script fallback is early-return-guarded.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03, **paused 2026-06-10**)_ — Slack briefing disabled at Tyler's request. Cron (was twice-weekly Mon + Thu 9:30 AM ET / UTC 13:30 — fatigue moves slowly, daily would over-query Meta) is commented out; `workflow_dispatch` still works. Apps Script fallback is early-return-guarded.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05, **paused 2026-06-08**)_ — Slack briefing (and the recurring Anthropic categorization spend) disabled at Tyler's request. Cron (was weekly Monday 10 AM ET / UTC 14:00) is commented out; `workflow_dispatch` still works — useful for refreshing the creative cache on demand. Weekly cadence matched the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:
...
GitHub Actions cron is best-effort — runs can be delayed, occasionally skipped, and **silently disabled after 60 days of repo inactivity**. To make scheduled runs more reliable, Apps Script time-based triggers (running on Google's cron infrastructure) act as a fallback. Functions in `apps-script/Code.js` (`triggerAgent*IfNeeded`) fire ~3 hours after the GitHub cron is supposed to run, check the GitHub API for a recent successful or in-progress run, and dispatch via `workflow_dispatch` only if none exists. If GitHub fired on time, Apps Script skips. If GitHub missed, the fallback picks it up. This two-scheduler pattern is live for `daily-data` (dual-purposed via `triggerAgentPipelineHealthIfNeeded`) and `agent-portfolio-scaling`. For the three paused workflows (`daily-check`, `fatigue-monitor`, `creative-intelligence`), both the GitHub cron **and** the matching `triggerAgent*IfNeeded` fallback are deliberately disabled (early-return guard) — pausing only the cron would leave the fallback re-dispatching the "paused" workflow within a few hours.
...
- **Campaign mapping is partially populated.** As of the last audit-snapshot export (**2026-04-23 — over two months stale as of this update; the branch is only refreshed by manually running `exportAuditSnapshot()`**), 4 of 21 campaigns in the mapping sheet had the "Prequal results page view" conversion event configured, and 3 had the newer "Investment Crowdfunding Prequal Decision" event. Campaigns without `custom_conversion_id` set fall back to Meta's generic lead conversion, which gives fuzzier ICP attribution. These counts likely drift as campaigns launch/pause — re-run `exportAuditSnapshot()` for a current read before treating them as precise.

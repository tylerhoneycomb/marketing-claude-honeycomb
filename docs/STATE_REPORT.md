...
- **`agent-daily-check.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-08**)_ — daily cron at 8:30 AM ET (UTC 12:30) is commented out; Slack briefings disabled at user's request. Manual `workflow_dispatch` still works. Re-enable by restoring the `schedule:` block and removing the early-return guard in `triggerAgentDailyCheckIfNeeded()`.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-10**)_ — twice-weekly cron (Mon + Thu 9:30 AM ET / UTC 13:30) is commented out; Slack briefings disabled at user's request. Manual `workflow_dispatch` still works. Twice-weekly cadence is intentional — fatigue moves slowly and daily would over-query Meta. Re-enable by restoring the `schedule:` block and removing the early-return guard in `triggerAgentFatigueMonitorIfNeeded()`.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05, **PAUSED 2026-06-08**)_ — weekly cron (Monday 10 AM ET / UTC 14:00) is commented out; Slack briefings disabled at user's request. Manual `workflow_dispatch` still works and is useful for refreshing the creative cache without autonomous Anthropic spend. Re-enable by restoring the `schedule:` block and removing the early-return guard in `triggerAgentCreativeIntelligenceIfNeeded()`. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:

...
_Last updated: 2026-09-15 (**The Tuesday scaling brief stopped proposing budget changes, and now only talks about campaigns that are actually running.** Tyler asked for both. The weekly brief is a diagnosis now: it says how each business vertical is performing and flags the ones that need a wider audience, and that is all. The budget reallocation it used to propose every Tuesday — and the Approve / Reject links that came with it — are switched off. The 3 AM job that applied approved reallocations is guarded so it cannot run, its leftover approval tokens are cleared, and an old Approve link still sitting in Slack now answers "strategic reallocation is retired" instead of scheduling a week-old budget move. Verticals whose campaigns are all paused are still computed and kept in the data file for history, but never appear in the brief. The scaling log sheet stops receiving rows; the weekly record of how each vertical was classified is now the repository's own file history for `data/derived/scaling_profiles.json`. What it means day to day: no Tuesday approval clicks, and — because the daily optimizer has been off since 2026-09-09 — no part of the system moves ad budget automatically any more. The reallocation code is unwired, not deleted, so it can be revived. Prior: 2026-09-14 **The dashboard no longer asks for a password.** The login screen that appeared when opening the Honeycomb Ads dashboard was removed at Tyler's request on 2026-09-14 — it was obstructing access and was only ever a browser-side check, not real protection: the data behind the dashboard comes from the Apps Script web app, which accepts requests from anyone with its address. Also on 2026-09-14: **Slack is now leads only.** On 2026-09-10 every Slack message was made lead-first but kept one secondary "of which N reached an IC decision" line; Tyler has since asked for leads only, so that line — and every other mention of investment crowdfunding, IC, ICP, CPICP, rewards crowdfunding or any subtype of leads — is gone from every Slack message and from the instructions behind the ones Claude writes. What remains in Slack is leads, cost per lead, spend and delivery diagnostics (click-through, frequency, CPM, pacing). The daily digest, the Monday narrative, the (paused) budget proposal and budget check, the new-campaign mapping alert, the pipeline-health alert and the four Slack-posting skill briefs were all changed; the health check now keeps a separate Slack-facing view so a problem with a subtype conversion is logged to the Sheet and the tracking issue but never posted to Slack. Nothing outside Slack changed: the Sheets still record the IC columns, the dashboard keeps its Investment Crowdfunding tab, the Hive Mind chat can still answer IC questions and the tracking-issue status lines keep their trailing IC token. Prior: 2026-09-10 **every Slack message made lead-first** — the daily "Honeycomb Ads" digest and the Monday narrative are retitled "Honeycomb Leads" and headline leads, CPL against the $16 target and spend, with the IC count demoted to one "of which N reached an IC decision" line; the campaign breakdown, the winners/bleeders lists and the Tuesday scaling brief all sort by CPL; every alert threshold is a CPL rule gated on a minimum lead count. The paused budget optimizer's ranking was rebuilt on CPL so it can be switched back on without shipping IC-first proposals (it stays off until Tyler signs off), and its stale approval tokens are cleared. The pass also fixed three things it found: the Tuesday brief's approve/reject links had never worked (the web app checked the optimizer's token first), the scaling portfolio total counted paused campaigns' budgets, and the messages still described the optimizer as running. Prior: 2026-09-09 **pivoted the whole system from Investment Crowdfunding conversions to leads**, then acted on three things the pivot surfaced. (1) **Turned off the daily budget optimizer** — it ranked campaigns by cost-per-IC, a number that fired once in 30 days, and was moving real money on it. Strategic reallocation from the weekly portfolio brief is unaffected. (2) **Closed the open `/exec` endpoint** — the web app was reachable by anyone with the URL, which is public in this repo; anyone could spend the Anthropic key, trigger a budget analysis, or queue budget rows and receive a valid approval token. Money-spending and state-changing actions now require a shared secret. (3) **Set the spend target to $300/day ($2,100/week)** to match the account's actual run rate, replacing a $10,000/week figure that no longer reflected anything. Prior: 2026-06-23 consolidated the pipeline-health check into `daily-data.yml`; 2026-06-10 PAUSED `agent-fatigue-monitor.yml`; daily-check, creative-intelligence and fatigue-monitor remain paused)_
...
- Posts the daily **"Honeycomb Leads"** Slack digest _(lead-first since 2026-09-10)_. Each of its three rows — yesterday, week-to-date, last 30 days — reads `Spend | Leads | CPL`; the week-to-date row compares CPL to the prior week and shows pacing against the weekly spend target, the 30-day row shows the $16 CPL target and a leads-per-week run rate, and yesterday's best and worst campaign by CPL are named. The one "watch" line picks, in order: a campaign that spent $50+ with zero leads, a campaign whose CPL is more than double the target on 5+ leads, or a campaign with frequency above 3.0. The two-sentence AI commentary is told to judge leads and CPL and to report leads, CPL, spend and delivery diagnostics only — never to split leads into categories or name any other conversion metric _(leads-only since 2026-09-14; the digest carries no IC line at all)_. The digest still annotates its data source — `rolling_data` sheet (this morning's pipeline snapshot) — so the user can reconcile against the parallel `daily-check` ad-level skill that fetches fresh from Meta later in the morning. The two reports may show different "yesterday spend" values because Meta's attribution can shift between the snapshot and the live read; the footer makes the source unambiguous.

### Every Monday at 8 AM (automatic)

- Picks the most recently completed week.
- Sends all the numbers to Claude (Anthropic's AI) with a prompt that asks for a short narrative: what happened, what to watch, what to do. _(Lead-first since 2026-09-10.)_ The numbers Claude sees open with total leads, overall CPL against the $16 target and spend; the campaign breakdown is sorted by CPL, best first, with no-lead campaigns last; the alert lists are "CPL above target" (more than 2× the target on 10+ leads), "CPL spike" (25%+ above the campaign's own 4-week average, 10+ leads), "frequency above 3.0" and "spend without leads". _(Leads-only since 2026-09-14.)_ The numbers carry no IC figure, and the prompt tells Claude to report leads, CPL, spend and delivery diagnostics only — never to split leads into categories or name any other conversion metric.
- Writes the narrative into a log sheet and posts it to Slack under the title **"Honeycomb Leads — Week of …"**, with yesterday / this week / last 30 days rows that read `Spend | Leads | CPL` (week-over-week and vs-prior-30 comparisons are on CPL), no IC line _(dropped 2026-09-14)_, and a **Budget Activity** block that separates daily-optimizer proposals from strategic reallocations, ties the block to the week's lead result, and reports honestly that neither is running — since 2026-09-15 its footer reads "Daily optimizer paused 2026-09-09; strategic reallocation retired 2026-09-15. No automated budget changes." instead of promising "next proposal: tomorrow morning". If the LLM call fails for any reason (HTTP error, malformed response, exception), the Slack post inlines the error detail (`[LLM call failed: HTTP 529: ...]`) instead of just pointing at the sheet — so Tyler doesn't have to open `intelligence_log` to see what went wrong.
...
- _(new, 2026-05-08; **retired 2026-09-15**)_ A second 3 AM job used to apply any **strategic reallocation** approved earlier in the week. Nothing proposes one any more, so the job now stops at its first line: it logs that strategic reallocation is retired, clears the leftover approval tokens and the optimizer-lockout markers, and returns. The trigger is still installed (so nothing has to be reinstalled if it is ever switched back on) but it can no longer touch a budget, post a reallocation summary, or honour an old Approve link. The guard exists because an executor with nothing feeding it is not harmless: a stale Approve link from a week before the retirement could otherwise have pushed a week-old budget move to live campaigns.
...
- Composes a two-section Slack brief titled **"Honeycomb Scaling — <date>"** _(lead-first since 2026-09-10; **diagnosis only since 2026-09-15**)_: it opens with one portfolio line (`N leads · $S spend · CPL $X` over 12 weeks) and a line saying what the live campaigns are spending per day and per week, then (1) lists verticals best-CPL-first within each class with their CPL and lead count, with verticals too new to classify getting a "too new to classify" one-liner instead of vanishing, and (2) lists audience action items — "duplicate the ad set with broader targeting" — for any vertical where frequency and CPM are both climbing. Nothing beyond leads, CPL, spend and delivery diagnostics (frequency, CPM, elasticity) appears in the brief _(the trailing IC line was removed 2026-09-14)_. The 12-week "median vertical CPL" figure was dropped on 2026-09-15 because it averaged across verticals that are mostly retired.
- **Only campaigns that are actually running appear.** A vertical whose campaigns are all paused is still computed and kept in the data file for history, but the brief never names, counts or alludes to it. Before 2026-09-15 the brief led with three all-paused verticals tagged "over-invested" while the only two campaigns Meta was delivering appeared last.
- **No budget proposal, no Approve/Reject links, no evaluation of last week** _(2026-09-15)_. The brief proposes nothing, registers nothing, and nothing executes afterwards. _(History: the approve links never worked at all until 2026-09-10 — the web app checked the daily optimizer's approval token first — so no strategic reallocation was ever approved and executed in production.)_
- One campaign-level fix from the earlier pass still applies: `LEADS-…` campaign names bucket into their vertical (before, the only two campaigns actually spending fell into one-campaign "verticals" and were skipped as too new), and the portfolio total counts ACTIVE campaigns only (before, paused campaigns' budgets made the total read ~$24k/week against a $2,100 target).
- The **12% weekly cap** per campaign still exists as a hard rail inside the daily optimizer's code — the total of all budget movement in a week cannot exceed 12%, which keeps Meta's ad-set learning phase from being reset by stacked changes. With the optimizer paused and the reallocation retired, nothing is currently contributing to it.
...
- **Budget controls** — run-analysis-now button (reports "paused" while the optimizer is off), adjust the weekly spend goal via a Slack approval flow. The spend-target Slack messages say the target is used by pacing (and by the daily optimizer once it is re-enabled) — the reference to the Tuesday reallocation was removed on 2026-09-15 when that reallocation was retired.
...
- **portfolio-scaling** _(shipped 2026-05-08; **diagnosis only since 2026-09-15**)_ — weekly Tuesday brief that adds a structural diagnosis layer on top of the (paused) daily optimizer. Classifies verticals over a 12-week window using elasticity (Pearson correlation of weekly spend vs weekly CPL), median-split CPL degradation between high- and low-spend weeks, and 4-week frequency/CPM trends, and flags the verticals whose audience is wearing out. One script (`compute_scaling_profiles.py`) commits deterministic JSON to `data/derived/scaling_profiles.json`, then Claude composes the two-section, lead-first Slack brief from that JSON (see "Every Tuesday" above). It only ever reports the verticals that still have a live campaign; all-paused verticals are kept in the same file under a separate key that the brief never reads. **It proposes no budget changes.** The pool-based reallocation it used to produce — decreases from saturating verticals, increases into scalable ones, bounded by the spend tolerance band — was retired on 2026-09-15 along with its Approve/Reject links, its Wednesday execution and its Wed–Mon optimizer lockout. The second script, `compute_reallocation.py`, is kept on disk but nothing runs it, so it can be revived without rewriting the maths. All thresholds still live in `data/config/benchmarks.json:scaling`. The audience action items no longer depend on the creative-intelligence cache at all (the "top body" prescription line went with the retired script; the `creative-intelligence-read` endpoint it would have needed was never built).
...
- **`agent-portfolio-scaling.yml`** _(shipped 2026-05-08; **diagnosis only since 2026-09-15**)_ — weekly cron Tuesdays at ~9:43 AM ET (UTC 13:43; minute moved off `:30` on 2026-06-23 to dodge GitHub's scheduled-run queue contention) — the only skill still on a schedule. One Python step runs as an ordinary workflow step, then commits `data/derived/scaling_profiles.json` BEFORE invoking `claude-code-action`. Claude reads the committed JSON, composes the two-section lead-first Slack brief, and posts it — that is the whole run. The reallocation step, the `/exec?action=scaling-queue-write` registration and the approval links were removed on 2026-09-15, and the workflow no longer receives the shared secret for the brief step because it makes no state-changing call. On the Apps Script side `executeStrategicChanges` still has its daily 3 AM trigger but returns immediately under a retirement flag, clearing any leftover approval tokens and lockout markers. Status comment to issue #48 opens with `leads=N cpl=$X`, counts only the verticals that were actually rendered, and ends `diagnosis_only`.
...
- **Human-in-the-loop safety.** No automated system pushes budget changes to Meta without a human clicking Approve in Slack, and two-step confirmation prevents Slack's link-unfurling bots from accidentally approving anything. As of 2026-09-15 there is nothing left to approve: the daily optimizer has been paused since 2026-09-09 and the weekly strategic reallocation is retired, so no automated budget movement remains at all. Both retirements also clear the approval tokens they leave behind, so an old Approve link in Slack cannot confirm something that will never run.
- **One metric everywhere.** Every Slack message, prompt and ranking in the system now headlines leads and CPL, sorts by CPL, and — since 2026-09-14 — mentions nothing else: no IC, ICP, CPICP, rewards or lead-subtype figure anywhere in Slack. The daily digest, the Monday narrative, the (paused) budget proposal, the Tuesday brief, the health alert and all five skill briefs read the same way.
...
## What changed on 2026-09-15, in plain terms

**The Tuesday brief only talks about campaigns that are actually running.**
Until today the weekly "Honeycomb Scaling" post ranked every business vertical
the 12-week window had ever seen — including verticals whose campaigns had all
been paused for weeks. The 2026-09-15 brief opened with three of those, tagged
"over-invested" with warning icons, while the only two campaigns Meta was
actually delivering came last. Verticals with no live campaign are still
computed and kept in the data file for history, but the brief never names,
counts or hints at them. A vertical comes back into the brief by itself as soon
as one of its campaigns is switched on again. The "median vertical CPL" figure
was dropped from the headline for the same reason: it averaged live performance
against campaigns that stopped running months ago.

**Budget reallocation is switched off entirely.** The brief used to size a pool
of dollars to move out of saturating verticals and into scalable ones, post it
to Slack with Approve and Reject links, and apply the approved moves at 3 AM.
All of that is gone: no pool, no proposal, no links, no execution, and no
evaluation of last week's moves. What remains is the diagnosis — how each live
vertical is performing, and which ones need a wider audience rather than more
money. The 3 AM job that used to apply approved reallocations now returns
immediately and clears the approval tokens and optimizer-lockout markers it
finds, and any old Approve link still sitting in a Slack channel answers
"strategic reallocation is retired" instead of scheduling a week-old budget
move against live campaigns.

**What it means practically.** There are no more Tuesday approval clicks — Tyler
reads the brief and decides what to do by hand. Because the daily budget
optimizer has been paused since 2026-09-09, this leaves the account with **no
automated budget movement at all**: every dollar that moves now moves because a
person changed it. The weekly record of how each vertical was classified moves
off the `scaling_log` sheet (which stops receiving rows) and into this
repository's own file history for `data/derived/scaling_profiles.json`, which is
committed on every Tuesday run. The Slack copy that used to promise a Tuesday
reallocation — in the Monday budget block and in the spend-target messages — was
corrected the same day, so nothing in Slack describes a reallocation that cannot
happen.

**Retired, not deleted.** The reallocation script is still in the repository
with a header explaining how to switch it back on, and the Apps Script executor
is guarded rather than removed. Reviving the feature means restoring a workflow
step and flipping a flag, not rewriting the maths.
...
cost per lead. The **weekly** strategic reallocation was untouched at the time
and kept running — it was retired separately on 2026-09-15 (see above).
...
dashboard once the API key is set), the daily check's pacing keeps measuring
against $9,000 — which is why it reads "underspending" every day. The Tuesday
scaling brief still reads the same setting into its data file but, since it
stopped proposing budgets on 2026-09-15, nothing is sized or bounded by it.

## What changed on 2026-09-14, in plain terms

**Slack is leads only.** The 2026-09-10 pass made every Slack message lead
with leads and cost per lead but let the IC count stay as one secondary
"of which N reached an IC decision" line. Tyler asked for that line to go.
Every Slack message — the daily digest, the Monday narrative, the paused
budget proposal and its "no changes" notice, the new-campaign mapping
alert, the pipeline-health alert and the daily-check, fatigue,
creative-intelligence and scaling briefs — now says nothing about
investment crowdfunding, IC, ICP, CPICP, rewards crowdfunding or any
subtype of leads, in any position. The instructions Claude follows when it
writes one of those messages no longer name those terms either; they say
"never split leads into categories and never name any other conversion
metric". What a Slack reader sees is leads, cost per lead, spend and
delivery diagnostics (click-through rate, frequency, CPM, pacing). The
"prequal decisions" count in the daily-check headline stays — it is the
leads funnel, not IC.

**Only Slack changed.** The Sheets still record the IC columns, the
dashboard keeps its Investment Crowdfunding tab and the Hive Mind chat can
still answer IC questions, the tracking-issue status lines keep their
trailing IC token, and the Sheet-write contracts still send the legacy
IC-named keys. The one structural change is in the health check: the
funnel-conversion check now keeps a Slack-facing view that covers the
quality-tier conversion only, so a subtype conversion that is archived or
stops firing is logged to the Sheet and the tracking issue but never
posted to Slack.
...
The IC count still appeared — once, as "of which N reached an IC decision" —
and never as the headline, the sort key or the reason something is flagged
(that line was removed on 2026-09-14; see above).
...
- **The scaling log sheet is frozen, with IC columns only.** `scaling_log` keeps `ic_rate` / `cpicp` and never gained a CPL or lead-count column. As of 2026-09-15 it also stops receiving rows entirely: the script that wrote to it went with the retired reallocation. The weekly record of how each vertical was classified is now the file history of `data/derived/scaling_profiles.json` in this repository, which is committed on every Tuesday run. The existing rows are still readable; nothing new lands in them.
...
- **Nothing has been deployed to Apps Script since the pivot.** The 2026-09-09, 2026-09-10 and 2026-09-14 `Code.js` changes (optimizer pause, `/exec` secret, lead-first then leads-only digest and narrative, the approve-link fix) ship only when the branch merges to `main` and the "Deploy Apps Script" workflow runs. Until then production still posts the IC-first digest and narrative every day, and the Tuesday links are still dead. Set `EXEC_SHARED_SECRET` in both places before merging, or every Sheet write fails closed.
...
5. **~~The Tuesday brief has never been exercised end-to-end.~~** _Moot 2026-09-15._ Its approve links were dead until 2026-09-10, so no strategic reallocation was ever approved and executed; the feature was then retired before anyone clicked one. `executeStrategicChanges` has never applied a budget change against Meta and, guarded, cannot. If the reallocation is ever revived, the first approval is still an untested path — test it on a single campaign.
...
- ~~Add `cpl` / `total_leads` columns to `scaling_log` so the Tuesday brief can evaluate last week from the log.~~ **Moot 2026-09-15** — the brief no longer evaluates last week and nothing writes to `scaling_log`. Only worth doing if the reallocation is revived.
...
This is a mature, working automation platform that now measures one thing — leads and cost per lead — and says so in every message it sends. The core data pipeline runs daily without intervention, budget changes go through two-step human-in-the-loop approval (Slack confirmation page defeats link-unfurling bots), and campaign renames in Meta are handled automatically without manual mapping cleanup. The Q1 audit's 6 data-quality issues are all resolved, the AI layer runs on Claude Opus 4.7 via a single configurable constant with lead-first prompts, and IC conversion tracking is intact but lives only in the Sheets and the dashboard's IC tab — since 2026-09-14 no Slack message mentions it. As of 2026-05-02, an additive ad-level agent layer (`scripts/`, `skills/`, `data/`) sits alongside the campaign-level pipeline — it gives Claude Code per-ad fatigue signals and creative metadata to power the monitor → detect → propose loop, while leaving every real budget change to a human. The daily budget optimizer is paused with a CPL-ranked replacement ready behind the guard, and as of 2026-09-15 the weekly strategic reallocation is retired as well — the Tuesday brief now diagnoses the verticals that are actually running and proposes nothing — so no part of the system moves ad budget automatically today. The biggest remaining items are operational: merging and deploying the pivot and the retirement, and a hard alert on lead volume collapsing.

...
_Last updated: 2026-05-14 (PR #79: creative-intelligence categorizer retry hardened — no longer brittle under load; PR #80: four code-quality fixes — overwrite protection on daily snapshots, new `below_floor` fatigue severity bucket, backfill status report fixed, DST drift documented in CLAUDE.md; PR #82: routing bug in `handleDashboardApi_` whitelist patched — first portfolio-scaling cron run on 2026-05-12 failed because `scaling-queue-read` and `scaling-log-read` were unreachable, fix deployed 2026-05-13)_
...
- **creative-intelligence** _(shipped 2026-05-05)_ — weekly Monday brief on what creative copy and visual patterns are winning across the portfolio. Tells Tyler what to write next by quoting actual winning copy alongside its real numbers (CPICP, IC count, ad count) and structural fingerprint (length, opening word, syntactic markers). The attribution model is corpus-level text aggregation: when the same body text appears across many ads, sum spend + IC across all of them to produce a meaningful per-variant CPICP. Three rounds of Meta API investigation proved that asset-level breakdown insights — the original spec's spine — won't return reliable per-variant conversion data for Honeycomb's `asset_feed_spec` ad mix; the design pivot is captured in [docs/CREATIVE_INTELLIGENCE_DESIGN.md](./CREATIVE_INTELLIGENCE_DESIGN.md). Two-script pipeline: `categorize_creative.py` (Anthropic API once per unique variant text + image, hash-deduped, ~$5 first run on Sonnet 4.5; retry hardened PR #79 — 5 attempts, exponential backoff, Retry-After header, error-type discrimination) and `build_creative_dataset.py` (joins snapshots + creative cache + categorizations, downloads full-size images via `/adimages` resolution, finds same-image-different-body side-by-side pairs). Writes per-vertical rollups to a new `creative_intelligence_log` Sheet tab. SKILL.md output rules require briefs that quote actual copy + cite real numbers + honor confidence labels — never recommend categories.
...
- **`agent-portfolio-scaling.yml`** _(shipped 2026-05-08; first cron run 2026-05-12 failed; routing bug fixed 2026-05-13, PR #82)_ — weekly cron Tuesdays at 9:30 AM ET (UTC 13:30). Two Python steps run as ordinary workflow steps, then commit `data/derived/scaling_profiles.json` + `reallocation.json` BEFORE invoking `claude-code-action`. Claude reads the committed JSON, registers the proposal via the `/exec?action=scaling-queue-write` endpoint to receive an approval token, composes the four-section Slack brief, and posts it. The execution side runs as `executeStrategicChanges` in Code.js — daily 3 AM trigger that no-ops cheaply when no pending strategic token exists. _(Bug note: the `scaling-queue-read` and `scaling-log-read` GET handlers were added to `handleDashboardApi_`'s switch but not to its allowlist, making them silently unreachable. The first scheduled cron run on 2026-05-12 failed when `compute_scaling_profiles.py` JSON-parsed an HTML error page. Fixed in PR #82 — two-line whitelist update.)_
...
- **Snapshot overwrite protection.** Re-running the daily-data workflow for a date that already has a snapshot now refuses with an error (exit code 2) instead of silently overwriting ground-truth data. Use `--force` / `FORCE=1` when a deliberate re-fetch is needed.
- **Fatigue signal fidelity.** The `below_floor` severity bucket distinguishes "ad has a fatigue signal but too little data to act on" from "ad is genuinely clean" — both used to land in the same `ok` bucket, making summary counts misleading.
- **Backfill reporting accuracy.** The daily-data workflow's status comment for backfill runs now correctly sums counts across all dates in the range instead of reading only yesterday's manifest.

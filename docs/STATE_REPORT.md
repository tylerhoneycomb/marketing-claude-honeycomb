...
_Last updated: 2026-07-10 (Documentation accuracy pass — no functional code changed since 2026-06-23; this update corrects drift in the docs themselves, not in the system they describe. Fixed: (1) the Apps Script line-count figure below was stale at "~4,200 lines" — the file is now ~6,500 lines, all added over time without anyone bumping this figure. (2) The "Every day at 6 AM" section previously said `agent-daily-check.yml`, `agent-fatigue-monitor.yml`, and `agent-creative-intelligence.yml` had "active cron" — all three have in fact been paused (workflow_dispatch only) since 2026-06-08/06-10; that pause was already correctly noted in CLAUDE.md but this report was never updated to match. (3) "Three skills are scoped" was leftover text from when the skill count was 3 — six skills now ship. See [TECHNICAL_REFERENCE.md](./TECHNICAL_REFERENCE.md) for the full list of corrections, including several stale/fabricated function-name citations. Prior: 2026-06-23 consolidated pipeline-health into daily-data.yml + de-peaked the two active crons)_
...
Skills are self-contained packages under `skills/<name>/` with a `SKILL.md` operating manual and Python scripts that handle Meta API calls and computation. Claude Code reads them at session start and runs the scripts via bash. Six skills ship today:
...
- **`agent-daily-check.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-08** — Tyler asked to stop the daily Slack briefing)_ — cron (was daily 8:30 AM ET / UTC 12:30) is commented out, not deleted; `workflow_dispatch` still works for manual runs. The Apps Script fallback trigger is early-return-guarded to match.
- **`agent-fatigue-monitor.yml`** _(shipped 2026-05-03, **PAUSED 2026-06-10** — Tyler asked to stop the Mon/Thu Slack brief)_ — cron (was twice-weekly Mon + Thu 9:30 AM ET / UTC 13:30 — fatigue moves slowly, daily would over-query Meta) is commented out, not deleted; `workflow_dispatch` still works.
- **`agent-creative-intelligence.yml`** _(shipped 2026-05-05, validated end-to-end 2026-05-05, **PAUSED 2026-06-08** — Tyler asked to stop the weekly Slack brief and the recurring Anthropic categorization spend)_ — cron (was weekly Monday 10 AM ET / UTC 14:00) is commented out, not deleted; `workflow_dispatch` still works and is useful for refreshing the cache on demand. Weekly cadence matched the corpus-aggregation attribution model. Three production runs on 2026-05-05 surfaced two distinct architectural findings that forced this skill's workflow to depart from the other agents' template:
...
- ~~**Hardcoded Claude model in 3 places.**~~ _Resolved 2026-04-22._ Extracted to `ANTHROPIC_MODEL` constant (Code.js:65). Upgraded to Opus 4.7. All 5 call sites reference the constant.

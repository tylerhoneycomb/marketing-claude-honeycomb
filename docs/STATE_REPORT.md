...
_Last updated: 2026-05-13 (PR #82: fixed `scaling-queue-read` + `scaling-log-read` missing from `handleDashboardApi_` whitelist, which broke the first scheduled Tuesday portfolio-scaling cron on 2026-05-12; PR #80 audit-cleanup: `fetch_ad_data.py` now refuses to overwrite existing snapshots without `--force`; `compute_signals.py` adds `below_floor` fatigue severity bucket; backfill status-comment DETAIL fixed)_
...
- **Portfolio-scaling skill routing fixed (2026-05-13).** The first scheduled Tuesday run (2026-05-12) failed because `scaling-queue-read` and `scaling-log-read` were missing from the `handleDashboardApi_` action whitelist, making both endpoints silently unreachable. Fixed in PR #82 (two-line add). The underlying class-of-bug (new `?action=` handlers require updates to both the switch case and the whitelist) is documented in the technical reference and tracked as an open refactor item.
...
5. **Dashboard API router has a silent whitelist trap.** The function that routes `?action=` requests maintains a hand-maintained allowlist. Adding a new skill that writes to or reads from the Google Sheet requires updating both the routing switch and the whitelist — missing one makes the endpoint silently unreachable (returns an HTML error page instead of JSON). PR #82 hit this and the fix was two lines; the risk is easy to re-introduce when adding new skills.

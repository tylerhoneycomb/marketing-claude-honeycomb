...
- `/apps-script/` — Full Apps Script intelligence layer, deployed via clasp + GitHub Actions
  - `Code.js` — The complete intelligence script (~6,400 lines). Edit here, never in the Apps Script web editor
...
- `/data/` — Agent data repository.
  ...
  - `data/creatives/` — creative metadata and LLM categorizations: `creatives.json` (accreted per-run), `categorizations.json` (Anthropic text + image tags, hash-deduped), `images/<hash>.jpg` (full-size creative images downloaded via Meta `/adimages`)
  ...
  - `data/drafts/` — human-reviewed ad copy output from the `ad-copy-generator` skill (`<date>-<vertical>.md`); never auto-published
  - `data/previews/` — free dataset preview Markdown files from `agent-creative-preview` workflow (no LLM cost)
...
- `.github/workflows/` — GitHub Actions CI/CD
  ...
  - `daily-data.yml` — Daily cron (8 AM ET / UTC 12:00) ad-level Meta pull; commits snapshots directly to main. Manual `workflow_dispatch` preserved for backfills via `start_date`/`end_date` inputs.
  - `agent-pipeline-health.yml`, `agent-daily-check.yml`, `agent-fatigue-monitor.yml`, `agent-creative-intelligence.yml`, `agent-creative-preview.yml`, `agent-ad-copy-generator.yml`, `agent-portfolio-scaling.yml` — Autonomous skill runners. See "Autonomous workflows" section below for schedules and details.
  - `claude.yml` — `@claude` mention handler for issues and PRs.

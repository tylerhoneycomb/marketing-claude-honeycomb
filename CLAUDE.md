...
- `.github/workflows/` — GitHub Actions CI/CD
  ...
  - `daily-data.yml` — Daily cron (~8:37 AM ET, UTC `37 12 * * *`) + workflow_dispatch for backfills. Fetches ad-level snapshots from Meta, computes derived signals, and runs merged pipeline-health checks (check_health.py + report_health.py) as deterministic steps at the end of the job.

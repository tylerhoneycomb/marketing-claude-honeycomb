...
- `agent-portfolio-scaling.yml` — runs `portfolio-scaling` skill.
  ...
  no-op days). **Known issue:** the issue-#48 status comment has
  posted `(no brief status)` on ~1/3 of recent Tuesday runs even
  though `data_commit=ok` — the deterministic Python steps and derived-JSON
  commit succeed, but `claude-code-action` intermittently doesn't
  write `/tmp/agent_status.txt` before exiting, so the Slack brief
  step silently produces no summary that run. Not data-loss (the
  scaling data itself is fine), but Tyler may miss a Tuesday brief.
  Tracked in TECHNICAL_REFERENCE.md §10.4.

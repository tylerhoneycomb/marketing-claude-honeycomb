...
function handleChatRequest_(e) {
  try {
    ...
    var systemPrompt = [
      ...
      'Some campaigns are optimized against the "Investment Crowdfunding Prequal Decision" Meta custom conversion event. These are called IC campaigns. When this event fires, Meta is telling us directly: this person became an ICP.',
      '',
      'IC campaigns do NOT use hybrid attribution. Their estimated_icps equals their ic_conversions count — a direct measurement from Meta. Their blended CPICP is computed as spend ÷ ic_conversions — ground truth, not an estimate.',
      '',
      'IC campaigns are completely excluded from the hybrid attribution pool. Their general lead conversions are not counted in totalConvByDate, and IC-generated ICPs are subtracted from the unattributed pool before non-IC campaigns divide it. Non-IC campaigns continue to use the hybrid attribution model as before. If an IC campaign has zero IC conversions in the window, it falls back to the hybrid estimate so it doesn\'t look artificially infinite.',
      '',
      'When comparing IC campaign CPICP to non-IC campaign CPICP, note that IC campaigns use a harder measurement (direct conversion count) while non-IC campaigns use an estimate (hybrid attribution). Both are valid but they\'re measuring slightly different things.',
      '',
      'IC campaigns can be identified by looking at the conversion_event column in campaign_mapping (value: "Investment Crowdfunding Prequal Decision") or by checking if ic_conversions > 0 in the rollup.',
      '',
